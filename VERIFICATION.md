...
- The public process binding accepts `processes.start.timeoutMillis` only from 0 through 60,000 and `processes.status.waitMillis` only from 0 through 10,000, displays both ranges to the model, and rejects longer waits before any durable operation or machine transport call crosses.
...
The cancellation-delivery case must additionally interrupt the API or Runtime worker after the durable cancellation request and after the first external `CellCancel`, but before acknowledgement persistence. Recovery must redeliver the same cancellation identity, prove the delayed side effect absent, and complete the next workspace call without replacing the Runner or Orb.
