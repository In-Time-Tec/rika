...
- In an Amp orb, `.agents/setup` installs pinned tools once and no `.agents/resume` hook exists because Rika has no quick wake repair. `.amp/services.yaml` alone owns the Docker daemon, credential service, `bun run dev` stack, `/readyz` check, and Portal; the application service waits boundedly for the concurrently declared daemon. `amp orb services ensure` or opening the Portal starts or repairs that declaration. A changed E2B template may outlive one ensure wait, so the same ensure command may be repeated while the original supervised service continues; no hook launches a duplicate stack or treats a configured Portal URL as readiness.
...
- Development template creation retries the same E2B alias and exact source digest at most three times only for recognized transient E2B 500/502/503/504, retryable upload-handshake, and transport failures. Authentication and deterministic validation failures do not retry.
- Production rejects missing production capabilities.
- Seed execution is development-only, idempotent, uses a real Better Auth password account, materializes Personal and Organization Owners through the real HostedProduct service, and stores encrypted OpenRouter credentials. Concurrent first runs serialize into one account, organization, membership, owner pair, and credential pair. A second run with the same key preserves every active credential persistence field; a changed key rotates exactly once under the same identity, and a revoked credential is restored with an advanced revision.
- PostgreSQL and MinIO images are immutable digest references. If `.alchemy` state is lost while either generated-secret volume remains, startup preserves the volume and reports the exact stale development resource instead of retrying forever, adopting with an invalid secret, or deleting data.
...
- A pending, preparing, paused, or active assignment on an old E2B build never reconnects that sandbox. One PostgreSQL transaction installs the currently approved exact build while incrementing generation and clearing old credentials, leases, provider identity, and capabilities. The old generation remains fenced. Replacement cannot change Orb provider scope or any Runner placement authority, and create, bootstrap, inventory adoption, and Executor hello all attest the successor generation's exact build.
...
- A terminal model provider failure that has already emitted authoritative `ModelCallFailed` evidence settles its exact durable model operation as failed and terminalizes the Run. It is never replayed, converted to `OperationUnknown`, cancelled, or repaired by a Rika event scan.
- Unknown Executor operation receipt remains `unknown` and is not blindly replayed. A terminal native-tool unknown records one never-replay Generalist marker, exposes accept/abort but not retry, and a later durable Executor terminal resolves that marker rather than the still-running outer operation.
- A hosted remote native tool operation is journaled as `provider-idempotent` only when its gateway durably deduplicates the exact `ToolContext.operationKey`; direct local tools remain `never`.
- API loss while a Runner or Orb native tool operation is active does not interrupt the tool effect, cancel the Run, fail the pending operation, or append `OperationUnknown`.
- Generalist re-enters the retry-safe tool with the same operation key, Rika attaches fresh operation authority to the existing dispatch, and the Executor replays the pending `MachineExecute` and terminal receipt without executing the tool twice.
...
- Executor process loss never reruns a persisted running native tool operation and produces an Executor-authored unknown result that remains operator-visible.
...
- Cancellation before Executor dispatch terminalizes the accepted executor operation without sending `MachineExecute`. Cancellation after dispatch sends a fenced `MachineCancel`, waits for the durable `MachineResult` terminal lifecycle frame, and exact repeated cancellation returns that same terminal.
- Cancelling a root with active Child Runs cancels every admitted descendant native tool operation, prevents later descendant tool admission, and leaves no late Workspace side effect.
- Cancelling a `bash` call while its machine request is active interrupts the delegated Workspace process, prevents a delayed write, and permits an immediate next machine call on the same assignment generation and process incarnation.
- The Orb control host cannot write the checkout, while its one persistent `rika-workspace` machine subprocess can write it and owns created files. The subprocess preserves shell process and MCP state across calls, receives only request-authorized runtime environment, and cannot modify root-owned `/opt/rika`. Local Runner tools remain in-process.
- The model-facing `bash.timeout_ms` accepts only 0 through 60,000 and `shell_command_status.waitMillis` accepts only 0 through 10,000. Their schema-derived descriptions show both ranges, and invalid waits are rejected before any durable operation or machine transport call crosses.
...
- After waiting longer than the command's original delay, use a fresh direct workspace inspection to prove the forbidden file does not exist. An interrupted or unknown inspection is not proof.
- Immediately execute another workspace process call on the same Thread and show that it succeeds without replacement preparation. Server evidence must confirm the assignment generation and Executor process incarnation did not change.
...
- after a local or Orb native tool operation is created but before the API receives it, and after the API persists a terminal native tool result but before the Executor receives it.
...
The cancellation-delivery case must additionally interrupt the API or Runtime worker after the durable cancellation request and after the first external `MachineCancel`, but before acknowledgement persistence. Recovery must redeliver the same cancellation identity, prove the delayed side effect absent, and complete the next Workspace call without replacing the Runner or Orb.
