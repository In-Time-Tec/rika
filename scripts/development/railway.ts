...
const remoteStageIdentity = ".alchemy/rika-dev-stage"
...
const parseRemoteStage = (value: string) => {
...
const readRemoteStage = Effect.fn("DevelopmentStack.readRemoteStage")(function* (root: string = process.cwd()) {
...
const ensureRemoteStage = Effect.fn("DevelopmentStack.ensureRemoteStage")(function* (root: string = process.cwd()) {
...
const runAlchemy = Effect.fn("DevelopmentStack.runAlchemy")(function* (operation: Operation, stage: string) {
  const args = alchemyCommand(operation, stage)
  const child = yield* spawnOwned(
    ChildProcess.make("alchemy", args, {
      cwd: process.cwd(),
      env: { RIKA_ALCHEMY_TARGET: "railway", RIKA_ALCHEMY_OPERATION: operation },
      extendEnv: true,
      stdin: "inherit",
      stdout: "inherit",
      stderr: "inherit",
...
const execute = (operation: Operation) =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const stage = yield* operation === "remote" ? ensureRemoteStage() : readRemoteStage()
      ...
      const exitCode = yield* runAlchemy(operation, stage).pipe(Effect.onExit(() => preserveRemoteState(stage)))
      yield* Effect.sync(() => {
...
const command = Command.make("rika-personal-railway").pipe(
