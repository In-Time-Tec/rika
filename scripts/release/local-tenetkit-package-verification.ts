...
export type PackedTenetKitPackage = {
  readonly manifest: string
  readonly directoryDigest: string
}

export type InstalledTenetKitPackage = {
  readonly name: string
  readonly directory: string
}

export const verifyInstalledTenetKitPackages = Effect.fn("LocalTenetKitSmoke.verifyInstalledTenetKitPackages")(
  function* (input: {
    readonly isolatedRoot: string
    readonly version: string
    readonly packedPackages: ReadonlyMap<string, PackedTenetKitPackage>
  }) {
    ...
    const storeEntries = (yield* fileSystem.exists(store))
      ...
      : []
    const installed: Array<InstalledTenetKitPackage> = []
