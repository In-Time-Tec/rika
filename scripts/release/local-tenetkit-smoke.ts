...
export type TenetKitReleaseEvidence = {
...
const failure = (step: string, message: string) => new LocalTenetKitSmokeError({ step, message })
...
const tenetkitReleaseInventoryErrorImpl = (
  evidence: TenetKitReleaseEvidence,
  version: string,
  checksumNames: ReadonlyArray<string>,
): string | undefined => {
  if (evidence.schemaVersion !== 1)
    return `Expected TenetKit evidence schema version 1; received ${evidence.schemaVersion}`
  ...
  if (JSON.stringify(actualPackages) !== JSON.stringify(expectedPackages))
    return "TenetKit evidence does not contain the exact current public package release train"
  ...
  if (JSON.stringify(checksumNames.toSorted()) !== JSON.stringify(expectedChecksums))
    return "TenetKit checksums do not contain exactly every package tarball and release-evidence.json"
  return undefined
}

export const tenetkitReleaseInventoryError: {
  (arg0: TenetKitReleaseEvidence, arg1: string, arg2: ReadonlyArray<string>): string | undefined
  (arg1: string, arg2: ReadonlyArray<string>): (arg0: TenetKitReleaseEvidence) => string | undefined
...
const manifestWithLocalTenetKitTarballsImpl = (
...
export const manifestWithLocalTenetKitTarballs: {
...
export const provisionProvenHostArchive = Effect.fn("LocalTenetKitSmoke.provisionProvenHostArchive")(function* (input: {
...
const program = (options: {
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const releaseDirectory = path.resolve(options.tenetkitRelease)
      ...
      const inventoryError = tenetkitReleaseInventoryError(
        evidence,
        version,
        checksumEntries.map(([filename]) => filename),
      )
      if (inventoryError !== undefined) return yield* failure("validate TenetKit release inventory", inventoryError)
      ...
      for (const [filename, digest] of checksums) {
        ...
        if (sha256(bytes) !== digest)
          return yield* failure("validate TenetKit checksums", `Digest mismatch for ${filename}`)
      }
      for (const item of evidence.packages) {
        if (checksums.get(item.filename) !== item.sha256)
          return yield* failure(
            "validate TenetKit evidence",
      ...
      const localManifest = manifestWithLocalTenetKitTarballs(sourceManifest, releaseDirectory, version)
      ...
      for (const consumer of ["packages/execution", "packages/extensions"])
      ...
      yield* Effect.log(`Local TenetKit tarball release smoke passed for Rika ${target} with TenetKit ${version}`)
    }),
  )

const command = Command.make(
  "local-tenetkit-smoke",
  {
