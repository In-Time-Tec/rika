...
export const make = (root: string): Isolation => {
  ...
  const tenetkitDatabase = HostFiles.join(absolute, "tenetkit.db")
  return {
    root: absolute,
    cwd,
    home,
    ...
    environment: {
      ...
      RIKA_TENETKIT_DATABASE: tenetkitDatabase,
...
export const assertSafe = (input: { readonly isolation: Isolation; readonly userHome?: string }): void => {
  ...
  for (const path of [isolation.root, isolation.cwd, isolation.home, isolation.temporary, isolation.tenetkitDatabase]) {
