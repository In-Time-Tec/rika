...
This benchmark exercises TenetKit's durable SQLite execution boundary with a custom `LanguageModel.make` stream. It compares published TenetKit 0.20.2 from a detached Rika v0.5.3 source identity with a complete local TenetKit 0.20.2 release inventory consumed from the current Rika source identity. It never uses `TestModel` parts as provider transport chunks.

Build the candidate release from the TenetKit candidate worktree:

```sh
rm -rf /tmp/tenetkit-semantic-release
mkdir -p /tmp/tenetkit-semantic-release
cd /Users/dallen.pyrah/projects/.worktrees/tenetkit-repl-kernel
PACKAGE_ARTIFACT_DIR=/tmp/tenetkit-semantic-release bun run package
```
...
bun run scripts/benchmark/semantic-output-benchmark.ts setup \
  --output /tmp/rika-semantic-output \
  --candidate-tenetkit-release /tmp/tenetkit-semantic-release
```
...
- The automated owning interface is TenetKit-only. It does not claim Rika transcript, projection, or OpenTUI measurements; `commitProjectionCalls` is zero and excluded from a ratio gate when both sources are zero.
...
- Provisioning published TenetKit 0.20.2 requires registry access and leaves the detached v0.5.3 worktree inside the chosen output directory. Removing the output may be followed by `git worktree prune`.
- Candidate numbers are valid only for the tarball digests in the recorded release evidence. A Git commit alone is not package identity, especially when the TenetKit candidate worktree contains uncommitted changes.
