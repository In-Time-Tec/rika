...
These are release-binary stretch targets on Apple Silicon at 120×36. Measurements use an isolated data-root copy and report client, resident, and combined resources separately. A result passes only when the supported performance runner produces the evidence; moving work to another process or machine does not count as removing it.
...
| Current-Thread selection                  |     ≤16 ms and no persistence or Relay read |
...
| Resident restart recovery, p95             |     ≤500 ms |

## Bounded work

- Persisted usage display reads one aggregate and never replays execution history.
- A reconciled terminal Thread replays zero Relay events and traverses zero historical tokens when reopened.
