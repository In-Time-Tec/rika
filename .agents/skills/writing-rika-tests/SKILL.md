...
- Build each test on `TuiApp.tuiApp({ script, ... })`: the real Surface on the OpenTUI test renderer, the real interactive loop from `apps/rika/src/main.ts`, and the real Operation, TenetKit, SQLite, and tool stack in one process.
