...
export interface HostedOperationsService {
  ...
  }) => Effect.Effect<ReadonlyArray<InteractiveEvent>, HostedOperationsError | ProductOperation.OperationUnavailable>
  readonly snapshot: (
    ownerId: OwnerId,
    threadId: ThreadId,
  ) => Effect.Effect<HostedThreadSnapshot, HostedOperationsError>
}
      ...
      interactive: (input) =>
        Effect.scoped(
          owners.contextEffect(input.ownerId).pipe(
            ...
            Effect.mapError((error) =>
              Schema.is(ProductOperation.OperationUnavailable)(error) || Schema.is(HostedOperationsError)(error)
                ? error
                : HostedOperationsError.make({ message: String(error) }),
            ),
          ),
        ),
          ...
          ownerRepositories.contextEffect(ownerId).pipe(
            Effect.flatMap((context) =>
              Effect.gen(function* () {
                ...
                if (thread === undefined) return yield* HostedOperationsError.make({ message: "Thread is unavailable" })
