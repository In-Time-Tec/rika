...
for (const review of [false, true]) {
  it.effect.skipIf(!live)(`commits and recovers the canonical ${review ? "review" : "ordinary"} Turn atomically`, () =>
    withAuthoritativeDatabase("atomic-run", (database) =>
      Effect.gen(function* () {
        ...
        if (review) Object.assign(input, { review: true })
        yield* Effect.tryPromise(() =>
          database.execute(
        ...
        const rolledBack = {
          ...
          turns: yield* Effect.tryPromise(() =>
            database.$count(rikaTurns, eq(rikaTurns.threadId, connection.threadId)),
          ),
        ...
        expect(firstClaim.preparedExecution.reviewIntent).toEqual(review ? reviewIntent(input.prompt) : undefined)
        ...
        expect(recoveredClaim.preparedExecution).toEqual(firstClaim.preparedExecution)
        ...
        expect((yield* runtime.list({ limit: 100 })).length).toBe(review ? 5 : 2)
