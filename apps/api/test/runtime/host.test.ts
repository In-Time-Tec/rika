import "./host-authorization.harness"
import "./host-lifetime.harness"
...
const workspace = unavailableWorkspace(workspaceBinding)
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          const host = yield* hostEffect({
            revision: "test-build",
            workspace,
            context: yield* threadContext(workspaceBinding, "rika-v2:owner:thread", model),
          })
          yield* activate
      ...
      const first = yield* Effect.scoped(
        Effect.gen(function* () {
          const host = yield* hostEffect({
            revision: "test-build",
            workspace,
            context: yield* threadContext(workspaceBinding, sessionId, model),
          })
          yield* activate
