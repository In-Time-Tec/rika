...
it.effect.skipIf(!live)("serializes controllers, replays cursors, and consumes socket tickets once", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      expect(yield* protocol.claimNextCommand({ claimToken: firstClaimToken, claimMillis: 60_000 })).toMatchObject({
        commandId: duplicate.commandId,
      })
      expect(
        yield* protocol.claimNextCommand({ claimToken: "duplicate-claim-busy", claimMillis: 60_000 }),
      ).toBeUndefined()
      yield* query(
        pool,
...
it.effect.skipIf(!live)("claims one admitted command, reclaims expiry, and includes Thread creation", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      expect(claims.find((claim) => claim !== undefined)).toMatchObject({ commandId: input.commandId })
      ...
      const recovered = yield* protocol.claimNextCommand({ claimToken: "worker-claim-recovered", claimMillis: 60_000 })
      ...
      ).toMatchObject({ commandId: "create-command", command: { _tag: "CreateThread" } })
    }),
  ),
)
...
it.effect.skipIf(!live)("claims ordinary commands in Thread version order across concurrent workers", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      expect(claims.filter((claim) => claim !== undefined)).toHaveLength(1)
      expect(claims.find((claim) => claim !== undefined)).toMatchObject({ commandId: first.commandId })
...
it.effect.skipIf(!live)("claims another Thread while one Thread command lane is locked", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      yield* protocol.initializeThread({ ownerId, threadId: otherThreadId, actor })
      ...
      yield* Effect.gen(function* () {
        ...
        expect(
          yield* protocol.claimNextCommand({ claimToken: "other-thread-claim", claimMillis: 60_000 }),
        ).toMatchObject({ threadId: otherThreadId, commandId: "other-thread-command" })
      }).pipe(
        Effect.ensuring(
...
it.effect.skipIf(!live)("does not let command cancellation overtake a non-prompt command", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      const first = { ...firstInput, command: { _tag: "PauseOrb", commandId: firstInput.commandId } }
      ...
      yield* protocol.admitCommand(second)
      expect(
        yield* protocol.claimNextCommand({ claimToken: "pause-before-cancel-claim", claimMillis: 60_000 }),
      ).toMatchObject({ commandId: first.commandId })
      expect(
        yield* protocol.claimNextCommand({ claimToken: "cancel-pause-claim", claimMillis: 60_000 }),
      ).toBeUndefined()
    }),
  ),
)
...
it.effect.skipIf(!live)("keeps event versions and snapshots monotonic when commands complete out of order", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      yield* protocol.admitCommand(second)
      expect(
        yield* protocol.claimNextCommand({ claimToken: "completion-first-claim", claimMillis: 60_000 }),
      ).toMatchObject({ commandId: first.commandId })
      expect(
        yield* protocol.claimNextCommand({ claimToken: "completion-second-claim", claimMillis: 60_000 }),
      ).toMatchObject({ commandId: second.commandId })
      ...
      expect(secondCompletion.command).toMatchObject({ threadVersion: "2", cursor: "1" })
      expect(firstCompletion.command).toMatchObject({ threadVersion: "1", cursor: "2" })
...
it.effect.skipIf(!live)("applies an admitted prompt without client traffic and recovers interrupted completion", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      const workerProtocol = ThreadProtocolStore.of({
        ...
        completeCommand: (input) => {
          ...
          return completionAttempts === 1
            ? Effect.fail(StoreError.make({ reason: "database", message: "simulated API interruption" }))
      ...
      yield* Layer.build(
        hostedThreadCommandWorkerLayer({ claimMillis: 10_000, pollIntervalMillis: 250 }).pipe(
          Layer.provide(
            Layer.mergeAll(
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        if (current.command.state === "completed") {
          ...
          expect(current.command.result).toEqual({ _tag: "ThreadCreated", threadId })
          break
        }
      }
      ...
      expect(replay.events).toMatchObject([{ event: { _tag: "SubmissionAdmitted", submissionId: "submission-1" } }])
    }),
  ),
)
...
it.effect.skipIf(!live)("lets command cancellation finish before a delayed prompt application", () =>
  withDatabase((pool) =>
    Effect.gen(function* () {
          ...
          Deferred.await(releasePrompt).pipe(
            Effect.map(() => {
              if (cancelledPrompts.has(input.operationKey))
                return { _tag: "Cancelled" as const, commandId: input.operationKey }
      ...
      yield* Layer.build(
        hostedThreadCommandWorkerLayer({ claimMillis: 10_000, pollIntervalMillis: 250, concurrency: 2 }).pipe(
          Layer.provide(
            Layer.mergeAll(
      ...
      const cancel = {
        ...
        command: {
          ...
          expectedThreadVersion: ThreadVersion.make("1"),
          target: { _tag: "Command", commandId: CommandId.make("delayed-submit") },
        },
      }
      ...
      expect(replay.events).toMatchObject([
        { event: { _tag: "ExecutionControlled", action: "cancelled", agentResponseArrived: false } },
      ])
    }),
  ),
)
...
it.effect.skipIf(!live)("resets compacted replica gaps and pushes contiguous events after listener recovery", () =>
  withDatabase((pool, url) =>
    Effect.gen(function* () {
      ...
      const replica = () =>
        Layer.build(
          hostedThreadProtocolLayerWithOptions({ databaseUrl: Redacted.make(url) }).pipe(Layer.provide(dependencies)),
        ).pipe(Effect.map((context) => Context.get(context, HostedThreadProtocol)))
      const [replicaA, replicaB] = yield* Effect.all([replica(), replica()], { concurrency: "unbounded" })
      ...
      const eventCursors = (frames: ReadonlyArray<{ readonly payload: { readonly _tag: string } }>) =>
        frames.flatMap((frame) =>
          frame.payload._tag === "ThreadEvent" && "event" in frame.payload
            ? [String((frame.payload as { readonly event: { readonly cursor: string } }).event.cursor)]
            : [],
        )
      yield* publish(2)
      ...
      expect(
        yield* connectionA.receive({
          protocolVersion: 1,
          requestId: "replica-a-ack" as never,
          command: { _tag: "AcknowledgeCursor", threadId, cursor: ThreadEventCursor.make("1") },
        }),
      ...
      expect(yield* connectionB.outbound).toMatchObject([
        { payload: { _tag: "ThreadSnapshot", cursor: "1", snapshot: currentSnapshot } },
      ])
  ...
  withDatabase((pool) =>
    Effect.gen(function* () {
      ...
      const product: HostedProductService = {
        ...
        admitAuthorizedRun: (input) =>
          Effect.sync(() => {
            if (!runs.some((run) => run.operationKey === input.operationKey)) runs.push(input as never)
            return {
              _tag: "Admitted" as const,
              commandId: input.operationKey,
              turnId: `turn-${input.operationKey}`,
      ...
      yield* Layer.build(
        hostedThreadCommandWorkerLayer({ claimMillis: 10_000, pollIntervalMillis: 250, concurrency: 8 }).pipe(
      ...
      const awaitCompletion = Effect.fn("ThreadProtocolStoreLiveTest.awaitCompletion")(function* (
        ...
        for (let attempt = 0; attempt < 40 && response[0]?.payload._tag === "CommandAdmitted"; attempt += 1) {
          yield* TestClock.adjust("250 millis")
          yield* Effect.yieldNow
          response = yield* session.receive({ ...message, requestId: `${message.requestId}:${attempt}` as never })
        }
        return response
      })
      ...
      expect(yield* awaitCompletion(controllerA, duplicate)).toMatchObject([
        { payload: { _tag: "CommandAccepted", threadVersion: "1", cursor: "1" } },
      ])
      ...
      expect(
      ...
      ).toMatchObject([
        {
          payload: {
            ...
            cursor: "3",
            snapshot: {
              pendingAuthorizations: [{ authorizationId: "authorization-1", turnId: "approval-turn" }],
            },
            events: [],
      ...
      const duplicateControl = {
        protocolVersion: 1 as const,
        requestId: "duplicate-control-a" as never,
        command: {
          ...
          target: { _tag: "Turn" as const, turnId: TurnId.make("denial-turn") },
        },
      }
      const duplicateControlResponses = yield* Effect.all(
        [
          controllerA.receive(duplicateControl),
          controllerB.receive({ ...duplicateControl, requestId: "duplicate-control-b" as never }),
        ],
        { concurrency: "unbounded" },
      )
