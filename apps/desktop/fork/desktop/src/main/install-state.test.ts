...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier OpenCode launch", () => {
    expect(hasExistingAppState([file("rika.settings")])).toBe(true)
    expect(hasExistingAppState([file("rika.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("rika")])).toBe(true)
  })
})
