...
import type { SecretVault } from "../hosted/hosted-credential-store"
...
export const makeRunnerReceiptStore = (options: {
  ...
  const load = Effect.fn("RunnerReceiptStore.load")(function* (scope: string) {
    ...
    const decoded = yield* Schema.decodeUnknownEffect(Schema.fromJsonString(SnapshotDisk))(stored).pipe(
