...
import { hardExit, makeExecutionControls } from "./resident-host-operation"
import type { Owner } from "./resident-host-operation"
import { makeConnectionHandler } from "./resident-host-connection"
import { isLegacyResidentPath, isResidentPath, protocolUpgradeReason } from "./resident-websocket-server"
import { makeInteractiveRouter } from "./resident-host-feed"
import { defaultOutboundCapacity, json } from "../protocol/resident-protocol"
    ...
    if (isLegacyResidentPath(request.url)) {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* writer(new Socket.CloseEvent(4403, protocolUpgradeReason))
        }),
      )
      return HttpServerResponse.empty()
    }
    if (!isResidentPath(request.url)) return HttpServerResponse.empty({ status: 404 })
    const socket = yield* request.upgrade
    yield* handle(socket)
    return HttpServerResponse.empty()
  })
