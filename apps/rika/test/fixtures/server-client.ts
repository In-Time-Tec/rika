...
const program = Effect.gen(function* () {
  ...
  const finalizerDelay = yield* Config.string("RIKA_TEST_SERVER_FINALIZER_DELAY").pipe(Config.withDefault("0"))
  const delayedWork = yield* Config.string("RIKA_TEST_SERVER_DELAYED_WORK").pipe(Config.withDefault("0"))
  const activeWorkMilliseconds = yield* Config.string("RIKA_TEST_SERVER_ACTIVE_WORK_MILLIS").pipe(
    Config.withDefault("0"),
  )
  const startupHold = yield* Config.string("RIKA_TEST_SERVER_STARTUP_HOLD").pipe(Config.withDefault("0"))
  const outboundCapacity = yield* Config.string("RIKA_TEST_SERVER_OUTBOUND_CAPACITY").pipe(Config.withDefault("1024"))
  ...
  const emit = Effect.fn("ServerClient.emit")(function* (value: unknown) {
  ...
  const kill = Effect.fn("ServerClient.kill")(
    function* (pid: number) {
    ...
    service.getOrCreate({
      ...
      startHost: () =>
        ServerProcessStartup.spawn({
          ...
          environment: {
            RIKA_TEST_SERVER_DATA_ROOT: dataRoot,
            RIKA_TEST_SERVER_GRACE: grace,
            RIKA_TEST_SERVER_FINALIZER_DELAY: finalizerDelay,
            RIKA_TEST_SERVER_DELAYED_WORK: delayedWork,
            RIKA_TEST_SERVER_ACTIVE_WORK_MILLIS: activeWorkMilliseconds,
  ...
  yield* runServerClientCommands({ connection, stdio, dataRoot, path, clock, fileSystem: fs, hostPid, emit, kill })
})

let supersedeStatusCount = 0
const statusLogger = Logger.make(({ message }) => {
  if (!Array.isArray(message) || !message.some((value: unknown) => String(value) === "server.startup.superseding"))
    return
