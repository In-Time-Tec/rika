...
test(
  ...
  () =>
    TuiApp.run(
      Effect.gen(function* () {
        const app = yield* TuiApp.tuiApp({
          // Cancellation interrupts the head before it answers; the promoted turn is the only
          // remaining model request, so the script must not reserve a third step for it.
