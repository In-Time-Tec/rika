...
import { LocalRunnerAdmission, type LocalRunnerRegistration } from "../src/local-executor/local-runner-contract"
import { liveAdmissionLayer } from "../src/local-executor/local-runner"
...
const registration: LocalRunnerRegistration = {
...
it.effect("registers the authenticated checkout, waits for admission, and revokes new assignments immediately", () =>
  Effect.gen(function* () {
    ...
    const http = Http.of({
      ...unusedHttp,
      registerLocalRunner: (_origin, fingerprint, runner) =>
    ...
    const admission = Context.get(context, LocalRunnerAdmission)
