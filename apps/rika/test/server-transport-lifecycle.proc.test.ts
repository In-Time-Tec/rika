      ...
      run(
        Effect.gen(function* () {
          ...
          try {
            ...
            expect((yield* fileStat(`${root}/server.token`)).mode & 0o077).toBe(0)
          ...
          try {
            ...
            yield* firstClient.awaitExit.pipe(
              Effect.timeoutOrElse({
                duration: "20 seconds",
                orElse: () => Effect.die("server startup owner did not exit after detaching its live host"),
              }),
            )
      ...
      run(
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(yield* fileExists(`${root}/server-${(yield* resolve("default", root)).identity}.startup`)).toBe(
              false,
            )
