...
import { CellTool, KernelPool, KernelProfile, TestKernel } from "@batonfx/repl"
import * as CellCallContext from "@rika/baton-execution/baton-cell-call-context"
import { configure } from "@rika/baton-execution/baton-route"
...
it.effect("mounts the rika surface as a dependency of the pool, so a cell can reach it", () =>
  Effect.gen(function* () {
    ...
    expect(Context.get(built, CellCallContext.CellCallContext)).toBeDefined()
...
it.effect("routes an admitted cell through the pool the composition root supplied", () =>
  Effect.gen(function* () {
    ...
    const pool = Layer.mergeAll(
      ...
      CellCallContext.layer,
    )
    const configured = yield* configure({
      executionRoute: testExecutionRoute(),
      workspace: "/workspace",
      kernel,
      kernelPool: yield* Layer.build(pool),
    })
