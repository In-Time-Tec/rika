...
const attached = (message: Message, value: HostedThreadSnapshot, cursor = "0"): Attached => ({
  ...
  threadId: message.command._tag === "AttachThread" ? message.command.threadId : ("invalid" as never),
  snapshotThreadVersion: cursor as never,
  snapshotCursor: cursor as never,
  threadVersion: cursor as never,
    ...
    layer: Layer.mergeAll(
      ...
      Layer.succeed(Socket.WebSocketConstructor, () => {
        const socket = new FakeWebSocket((current, message) => {
          ...
          receive(current, message, harness)
        })
        sockets.push(socket)
      ...
      Layer.succeed(
        ...
        Http.of({
          ...
          issueThreadTicket: () =>
            Effect.succeed({
              ticket: "ticket",
              expiresAt: "2026-08-25T01:00:00.000Z" as never,
...
const runSession = Effect.fn("test.runSession")(function* (
  ...
  const states: Array<typeof hosted.connection.initialState> = []
  const stateFiber = yield* Stream.runForEach(hosted.connection.stateChanges, (state) =>
    Effect.sync(() => states.push(state as never)),
  ...
  return {
    ...hosted,
    states: states as Array<{ target: string; activity?: string; connectivity: string }>,
    stateFiber,
    eventFiber,
  }
})
        ...
        socket.frame(
          attached(
            ...
            waitingSnapshot("orb", {
              ...
              ...(state === "failed" ? { message: "checkout failed" } : {}),
            }),
            String(attachCount),
          ),
        )
    ...
    Effect.gen(function* () {
      ...
      const harness = makeHarness((socket, message) => {
        ...
        socket.frame({
          ...
          cursor: String(version) as never,
          result: { _tag: "Applied" },
        })
      })
      const hosted = yield* runSession(harness)
        ...
        socket.frame(
          submissions.length === 1
            ? {
                ...
                threadId: message.command.threadId,
                reason: "unavailable",
                currentThreadVersion: "1" as never,
        ...
        if (message.command._tag === "AttachThread") {
          ...
          if (attachments === 2) {
            socket.frame({
              ...
              snapshotThreadVersion: "0" as never,
