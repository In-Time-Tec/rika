...
import { inspectRunnerCheckout } from "../src/runner/runner-checkout"
import { RunnerRegistration } from "../src/runner/runner-contract"
...
it.effect("binds opaque, distinct checkout identities to the authenticated device", () =>
  withPlatform(
    Effect.gen(function* () {
      ...
      const first = yield* inspectRunnerCheckout({
        deviceId: "device-1",
        workspace: firstPath,
        remoteThreadCreation: "denied",
      })
      const second = yield* inspectRunnerCheckout({
        deviceId: "device-1",
        workspace: secondPath,
        remoteThreadCreation: "denied",
      })
      const serialized = yield* Schema.encodeEffect(Schema.fromJsonString(RunnerRegistration))(first.registration)
