...
import { makeRoot, run } from "./server-transport-runtime"
import { cleanRoot, readText } from "./server-transport-files"
import { attachedEffect, nextTypeEffect, start } from "./server-transport-process"
...
describe("server WebSocket process transport", () => {
      ...
      run(
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(event.error).toContain("16 MiB server message limit")
  ...
  test(
    "degrades more than 64 critical events and keeps commands usable",
    () =>
      run(
        Effect.gen(function* () {
          const root = yield* makeRoot
          try {
            const client = yield* start(root, 350, 0, false, 1)
            yield* attachedEffect(client)

            yield* client.send("critical-overflow")
            expect(yield* client.nextEffect).toEqual({
        ...
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(event).toMatchObject({
              ...
              outcome: "Success",
              tags: expect.arrayContaining(["ThreadViewPatch", "ThreadsListed"]),
            })
            yield* client.closeEffect
          } finally {
            yield* cleanRoot(root)
  ...
  test(
    "detaches and replaces the one server event consumer",
    () =>
      run(
        Effect.gen(function* () {
          const root = yield* makeRoot
