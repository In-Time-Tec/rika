...
describe("server WebSocket process transport", () => {
  test(
    "cancels interrupted client work while keeping the server connection usable",
    () =>
      run(
        Effect.gen(function* () {
  ...
  test(
    "keeps one interactive callback and restores reads across server replacements without retrying mutations",
    () =>
      run(
        Effect.gen(function* () {
          const root = yield* makeRoot
