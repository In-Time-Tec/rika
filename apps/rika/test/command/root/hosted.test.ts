...
it.effect("routes retained account commands and V2 Thread operations without the local server", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      expect((yield* Effect.exit(invoke(["thread", "recovery", "inspect", "thread-1", "run-1"])))._tag).toBe("Failure")
      expect((yield* Effect.exit(invoke(["credential", "list", "--scope", "user"])))._tag).toBe("Failure")
      expect(yield* Ref.get(productCalls)).toEqual([])
      expect(yield* Ref.get(hostedCalls)).toHaveLength(16)
    }),
  ),
)
