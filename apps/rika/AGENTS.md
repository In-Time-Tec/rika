...
Thin Effect CLI shell and process composition root. Leaf command modules export command values. `src/command/root/rika-command.ts` exports the root command and testable `run(argv)`. `src/command/root/noninteractive-run-command.ts` owns JSONL parsing. `src/client-main.ts`, `src/interactive-main.ts`, and `src/performance-main.ts` interpret their isolated process programs.
