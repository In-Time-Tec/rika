...
verify_archive_inventory() {
  ...
  expected="$(printf '%s\n' "${archive_root}/" "${archive_root}/INSTALL" "${archive_root}/bin/" "${archive_root}/bin/rika" "${archive_root}/bin/.rika-client-runtime" "${archive_root}/bin/.rika-kernel-runtime" "${archive_root}/bin/.rika-kernel-worker.js" | LC_ALL=C sort)"
...
[ -x "${staging}/${archive_root}/bin/.rika-client-runtime" ] || fail "release archive is missing the client runtime"
