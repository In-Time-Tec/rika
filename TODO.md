...
- [x] WI-3 `rika-thread` entity kind + `ensureThreadHost`: `packages/runtime/src/thread-host.ts` registers the host agent (`inbox_enabled`, wait/send permissions, deterministic `rika/thread-host` model) and the `rika-thread` kind (drain all, state disabled, continue-as-new 32 < host max wait turns); entities are created named per ThreadId. Deviation: `getOrCreateEntity` is forked and completion is observed by polling `getEntity`, because the published 0.1.0 embedded runtime never resolves start-await for an execution that parks (fixed upstream on the unreleased fix/sqlite-sdk-package branch); the fork+poll deletes when that release lands.
- [x] WI-4 Pending-turn promotion through the durable inbox: submit while busy sends `{kind: "pending-turn", thread_id, turn_id}` with idempotency key `rika:turn:<turnId>`; the parked host wakes on `wait_for_messages`, calls `promote_turn`, and the registered promoter runs `TurnRepository.claimNextQueued`, which stays the exactly-once arbiter. Steering remains `client.steer`. Deviations: completion re-notification uses time-scoped keys `rika:nudge:<threadId>:<now>` instead of the audit's re-send-same-key (a deduplicated same-key nudge would never wake the host again and strands the queue); notifies are serialized behind a park-before-send guard (poll `inspectExecution.waiting_on` + semaphore + terminal self-heal) because a send landing while the wait tool pins its event sequence fails the host execution with `EventLogError` in published 0.1.0 — guard deletes with the fix/sqlite-sdk-package release, and the race is worth filing upstream.
