...
**Rejected:** interactive gates add friction to every ordinary edit; workspace containment blocked real work without stopping `bash`; a long command blocklist produces false positives on legitimate `sudo`, `dd`, `mkfs`, and force-push work while giving false confidence; an OS sandbox is a host concern rather than a Rika one.
