...
**Gain:** native tools work on any path the user can reach, so sibling repositories, dotfiles, absolute paths, and miscased paths stop failing. A short hardcoded refusal list still stops the few mistakes nobody recovers from.
...
**Rejected:** interactive gates add friction to every ordinary edit; workspace containment blocked real work without stopping a shell command; a long command blocklist produces false positives on legitimate `sudo`, `dd`, `mkfs`, and force-push work while giving false confidence; an OS sandbox is a host concern rather than a Rika one.
