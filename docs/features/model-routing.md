...
`modelRoutes.modes` sets the main and Oracle route for a mode. A role accepts an alias name, or an object with `alias` and an optional `effort` and `fast`; omitted values keep the mode's built-in policy. `modelRoutes.title` routes thread-title generation the same way, and `modelRoutes.agents` routes an individual specialist by agent id. Agent routes resolve through configuration but are not yet applied to execution, which configuration reports as a diagnostic. Compaction summaries are fixed to Sol/xhigh.
