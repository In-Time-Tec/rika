...
Rika Server is the hosted control plane. Stateless Bun and Effect replicas serve HTTPS, authenticated client WebSockets, and authenticated executor WebSockets. PostgreSQL owns product state and TenetKit's runtime state; replicas coordinate queue admission, model execution, executor assignments, projections, E2B lifecycle, and durable delivery through transactions and leases.
