...
Runner and Orb Executors connect outbound and expose only their assigned Workspace capability. They own files, native tool effects, services, and processes while their current generation and lease remain valid. The FoldKit web app and OpenTUI client consume the same snapshots, events, presence, and lifecycle commands. Executors never open PostgreSQL, admit a Generalist Run, or become a fallback Server; disconnecting a client never stops shared work.
