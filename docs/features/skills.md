...
`rika skills list` and `inspect` expose discovered skills. `add` copies an existing skill directory into the Workspace without overwriting another skill, and `remove` deletes the named Workspace skill. Skill files inform admitted agent instructions; they do not add model-visible workspace tools.
