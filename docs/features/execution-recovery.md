...
Rika fails a nonterminal Turn whose durable Execution link or Run is unavailable, records an execution-unavailable transcript unit, and drains queued Turns behind it. A linked Turn with a transient inspection failure remains observed. A Turn whose Execution reports no projection or model-preview progress for fifteen minutes fails with an execution-stalled transcript unit instead of remaining active indefinitely.
