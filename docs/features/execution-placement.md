...
A local-device foreground executor advertises `pty: false`. It evaluates dispatched cells as one-shot shell commands in the user's workspace and does not accept, persist, or replay a live interactive PTY. Durable PTY sessions remain an E2B executor capability.
