...
The Rika Server drains each Thread's queue directly after active work settles with any terminal outcome, including failure: a failed Turn never blocks the queued Turns behind it, and a queued Turn that itself fails does not stop the drain. Claims are durable and safe to retry after interruption. A claimed Turn emits `TurnStarted` only after preparation succeeds and it enters `running`; before that event it appears only in the queue, never as a transcript message. Users may edit, remove, or steer queued text into the active Execution before that point.
