...
Baton 0.4.2 registers exact model selections but exposes no candidate chain whose fallback is limited to availability failures before output. Relay's schema-backed contract carries typed compaction policies on root definitions, child presets, and fan-out overrides.
