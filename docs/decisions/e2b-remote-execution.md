...
E2B supplies isolation, process and PTY lifecycle, filesystem-only pause and resume, and replaceable workspace state. PostgreSQL remains authoritative for the Thread, access, command order, assignment generation, cursors, and checkpoint manifests; Generalist remains authoritative for Runs. Verified workspace artifacts live in object storage. A provider snapshot is never the recovery contract. This lets a sandbox be paused, replaced, or fenced without changing Thread identity or accepting writes from two executors.
