...
When a launching client and the running Server carry different build identities, the build the user just launched supersedes the Server after authenticated negotiation. Replacement closes mutation admission, drains already-admitted transient requests, and transfers sole ownership to the launched build. A durable running Execution does not veto this transfer; the replacement runtime recovers it from Generalist. Existing interactive clients keep their stable interface callback, reconnect through the one current transport contract, restore their selection, and resume reads from durable state.
