...
Supersession is a property of the launch, not of incompatibility. Before this rule, every client that saw an incompatible resident tried to replace it, so an upgrade with two live sessions became a kill war that ended in failed clients. Version-namespaced side-by-side residents were rejected because one resident is the single execution and persistence owner for a Profile and data root; two versions cannot share `rika.db` and the current execution store.
