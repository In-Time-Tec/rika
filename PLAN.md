...
The complexity test applied to every repair is: fix the first false owner, keep one durable fact, and make loss converge by identity. A new queue, cache, retry loop, status table, provider abstraction, or cleanup lease is rejected unless it protects a side effect that existing authority cannot fence. That is why the design uses one Thread WebSocket instead of SSE plus a mutation channel, PostgreSQL instead of PostgreSQL plus a broker, TenetKit cancellation instead of history scanning, and immutable provider IDs instead of a second durable scheduler merely to deduplicate harmless `kill(id)` retries.
...
Every owner-and-Thread-scoped command session must select its initial Thread and receive that Thread's first `ThreadViewSnapshot` before it is ready to process a command. Starting the event feed alone is not readiness: without selection, cancellation has no selected Thread and can return a cosmetic acknowledgement without reaching the linked TenetKit Run.

The hosted application now starts the feed, calls `selectThread(threadId)`, and completes readiness only from the matching snapshot. An initial `ExecutionFailed` or feed failure fails readiness. This creates one causal order—select, observe, then command—rather than letting every control operation compensate for a half-initialized session. Cancellation backend failures become `ExecutionControlFailed(action = cancel)`, and the command worker persists the matching Cancel command as rejected instead of reporting it applied.
...
The sandbox itself receives only the phase-specific setup or runtime egress allowlist plus the control WebSocket hostname, followed by an explicit deny-all rule. Public inbound traffic is disabled. Setup can reach declared repository/package hosts, runtime narrows to the authorized tool policy, and neither phase can use metadata, private-network, wildcard, raw-IP, or arbitrary public destinations. The tunnel narrows what can reach the API; the E2B network policy independently narrows what the Executor can reach.

The restricted listener starts first and must echo the new Caddy process's random readiness identity before its public URL is published to Alchemy. Its HTTP site label accepts any `Host` while an explicit `bind 127.0.0.1` keeps the socket loopback-only. Putting `127.0.0.1` in the site label would also make it a Host matcher: Cloudflare's public hostname would miss every route and receive Caddy's empty default response instead of the deny-by-default policy. Readiness therefore probes through a deliberately different Host as well as a random private header. A stale, foreign, or host-restricted listener cannot satisfy that proof. Without the matching private request header, the readiness path remains the same 404 as every non-Executor route. Cloudflared then emits and validates one HTTPS quick-tunnel URL. The dependent API starts with the corresponding immutable WSS Executor URL. If the URL changes, Alchemy restarts the API. One narrow signal-forwarding wrapper may discard cloudflared's localhost diagnostic URL and print only a validated `https://*.trycloudflare.com` value because `Command.Dev.url` scans output for only five seconds; it will not supervise, retry, or retain state.
...
Registration, random durable command allocation, and publication into the rendezvous are one uninterruptible step. A second in-flight call with the same UI submission identity is rejected before it sends another durable command. The sending-state transition and physical WebSocket send attempt are also uninterruptible: interruption before that boundary removes the unsent identity, while interruption after it retains the identity because admission may have happened. Definitive command rejection removes the rendezvous in the socket receive path before the waiting submit fiber can observe that result. The client remembers delivered cursors per Thread across attachment switches. An attachment must first pass cursor and Thread-version non-regression validation before its events may reconcile pending submissions; a stale rejected attachment cannot erase the correlation needed by Ctrl+C. Only explicit matching admission/rejection events from an accepted stream retire cancellation correlation. A newer Thread snapshot may reflect an unrelated event while an admitted submit command is still pending, so a Thread-global cursor never guesses that command's outcome.

### Render failures with the Effect CLI error grammar

Failures are durable transcript information, not success-shaped toasts or temporary status text. The TUI renders every user-relevant failure as the same two-line grammar used by Effect CLI's default formatter: a bold red `ERROR` heading followed by a red two-space-indented message. The message combines the operation title and typed detail without an expand affordance that reveals no additional information.

Submission, steering, cancellation, approval, denial, workspace, execution, and transport failures all enter the structured Error block path. Informational notifications and successful actions may still use ordinary notifications or toasts; painting a toast red does not turn it into an error surface.
...
PostgreSQL triggers issue `NOTIFY(threadId)` in the same transaction as protocol events, transcript projection checkpoints, event-backed snapshots, Turn status transitions, and workspace placement transitions. Every API replica owns one supervised `LISTEN` connection and preserves the payload: a notification wakes only sockets attached to that Thread. The attachment captures its Thread generation before reading durable state, so publication racing the read cannot be missed. Listener establishment and re-establishment wake every local cursor once, so a notification committed while that replica was not listening converges through durable cursor replay. There is no periodic replica sweep and no per-socket timer.

A transcript checkpoint notification means only that committed product state may have changed. On that wake, the socket rematerializes `HostedThreadApplication.snapshot`, compares it with the durable protocol snapshot at the authoritative Thread version and event head, and writes a same-cursor replacement only when their canonical fingerprints differ. The exact version/cursor fence retries if an interactive event advanced concurrently. It then rereads and sends only durable protocol state. The snapshot write's own notification is harmless because the next comparison is equal, so the push path converges without a notification loop or projection polling.
...
- Before creating an E2B sandbox, inventory the exact app, deployment, assignment generation, template, and build. Reconnect one usable deterministic match, bind it durably, and route every duplicate through PostgreSQL-authorized orphan inspection, grace, claim, and fenced cleanup. PostgreSQL-clock bootstrap liveness decides whether generation-matching provisioning sandboxes remain protected; unknown ownership leaks conservatively instead of being deleted.
