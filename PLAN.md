...
Unfinished: the Darwin-only resource-measurement gates are gone — main deleted the idle-gate
suite outright, and `performance-platform` is skipped unless `RIKA_PERFORMANCE_GATE=1` because
the hosted-account shift makes the interactive client exit before it spawns its
