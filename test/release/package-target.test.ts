...
it.layer(BunServices.layer)("release target construction", (test) => {
  ...
  test.effect("builds one public client with a private kernel runtime", () =>
    Effect.gen(function* () {
      ...
      expect(packaging).not.toContain("compileLauncher")
      expect(packaging).not.toContain("client-launcher.c")
