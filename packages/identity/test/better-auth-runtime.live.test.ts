...
const dpopProof = Effect.fn("IdentityLiveTest.dpopProof")(function* (input: {
  ...
  const payload = {
    ...
    htm: input.method,
    htu: input.url,
    iat: Math.floor((performance.timeOrigin + performance.now()) / 1_000),
...
it.effect.skipIf(!live)("uses the reviewed PostgreSQL schema for identity and DPoP OAuth device flows", () =>
  Effect.gen(function* () {
    ...
    const server = Bun.serve({
      ...
      fetch: (incoming) =>
        runtime === undefined
          ? Promise.resolve(new Response("Not ready", { status: 503 }))
    ...
    try {
      ...
      const registration = (yield* Effect.promise(() => registered.json())) as { readonly client_id: string }
      ...
      const authorization = (yield* Effect.promise(() => device.json())) as {
      ...
      const tokens = (yield* Effect.promise(() => token.json())) as {
