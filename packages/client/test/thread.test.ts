...
const product = (): ProductClient => ({
  ...
  createThread: () => Effect.die("Thread creation is not used by ThreadClient tests"),
  archiveThread: () => Effect.die("Thread archive is not used by ThreadClient tests"),
...
it.effect("steers the canonical active Run and never cancels it when the client closes", () => {
  ...
  return Effect.gen(function* () {
    ...
    const selected = client.state.threads[0]
    yield* client.refreshThreads()
    expect(client.state.threads[0]?.pending).toEqual(selected?.pending)
    expect(client.state.threads[0]?.activity).toBe(selected?.activity)
...
it.effect("ignores an older Thread list response after a newer refresh completes", () =>
  Effect.gen(function* () {
        ...
        listThreads: () => {
          ...
          if (calls === 1)
            return Deferred.succeed(started, undefined).pipe(
              ...
              Effect.as({ threads: [{ id: "old", title: "Old Thread", target: "runner" as const }], nextCursor: null }),
            )
          return Effect.succeed({
            threads: [{ id: "new", title: "New Thread", target: "orb" as const }],
            nextCursor: null,
          })
        },
      },
      execution: {} as ExecutionClient,
    })
    const previous = yield* client.refreshThreads().pipe(Effect.forkChild)
    yield* Deferred.await(started)
    yield* client.refreshThreads()
    yield* Deferred.succeed(release, undefined)
    yield* Fiber.join(previous)
    expect(client.state.threads.map((thread) => thread.id)).toEqual(["new"])
    yield* client.dispose
  }),
)
...
it.effect("rejects an in-flight child open after Thread selection changes", () => {
  ...
  const execution = {
    ...
    family: ({ sessionId }: { readonly sessionId: string }) =>
      Effect.succeed({
        ...
        at: 1,
        sessions: sessionId === "session" ? [retainedChild] : [],
        nextBefore: null,
      }),
    submit: () => Effect.succeed({ id: "receipt", revision: 1 }),
    updateInput: () => Effect.succeed({ id: "pending", revision: 2 }),
  ...
  const productForSwitch: ProductClient = {
    ...
    ensureSession: (threadId) =>
      Effect.succeed({ sessionId: threadId === "thread" ? "session" : "other-session", created: false }),
  }
  const client = makeThreadClient({
...
it.effect("subscribes to the focused child and stops that connection on back", () => {
  ...
  const execution = {
    raw: {},
    snapshot: ({ sessionId }: { readonly sessionId: string }) =>
      Effect.succeed(sessionId === "session" ? rootSnapshot : childSnapshot),
    history: () => Effect.succeed({ leafId: null, entries: [], nextLeafId: null }),
    family: () => Effect.succeed(family),
    submit: () => Effect.succeed({ id: "receipt", revision: 1 }),
    updateInput: () => Effect.succeed({ id: "pending", revision: 2 }),
...
it.effect("keeps the later Thread selection when an older connection teardown is held", () =>
  Effect.gen(function* () {
    ...
    const connectionFor = (sessionId: string) => ({
      snapshot: snapshotFor(sessionId),
      events: sessionId === "first-session" ? Stream.never : Stream.empty,
      status: Stream.empty,
      exhausted: Effect.never,
      cancel: () => Effect.void,
    })
    const execution = {
      ...
      connect: ({ sessionId }: { readonly sessionId: string }) =>
        sessionId === "first-session"
          ? Effect.acquireRelease(Effect.succeed(connectionFor(sessionId)), () =>
              Deferred.succeed(finalizerStarted, undefined).pipe(
                Effect.andThen(Deferred.await(releaseFinalizer)),
                Effect.asVoid,
              ),
            )
          : Effect.succeed(connectionFor(sessionId)),
      subscribe: () => Stream.empty,
    } as unknown as ExecutionClient
    const client = makeThreadClient({
      product: {
        ...
        listThreads: () =>
          Effect.succeed({
            threads: [
              { id: "first", title: "First", target: "runner" },
              { id: "second", title: "Second", target: "runner" },
              { id: "third", title: "Third", target: "runner" },
            ],
            nextCursor: null,
          }),
        thread: (id) => Effect.succeed({ id, title: id, target: "runner" }),
        ensureSession: (threadId) =>
          Effect.sync(() => {
            ensuredThreads.push(threadId)
            return { sessionId: `${threadId}-session`, created: false }
          }),
      },
    ...
    yield* client.selectThread("first")
    yield* waitForDetached(10)
    const older = yield* client.selectThread("second").pipe(Effect.forkChild)
    yield* Deferred.await(finalizerStarted)
    const later = yield* client.selectThread("third").pipe(Effect.forkChild)
    yield* Fiber.join(later)
    yield* Deferred.succeed(releaseFinalizer, undefined)
    yield* Fiber.join(older)
    expect(factoryCalls).toEqual(["first", "third"])
    expect(ensuredThreads).toEqual(["first", "third"])
    expect(client.state.selectionEpoch).toBe(3)
    expect(client.state.selectedThreadId).toBe("third")
    expect(client.state.selectedSessionId).toBe("third-session")
    yield* client.dispose
  }),
)
