import { Config, Crypto, Effect, FileSystem, Ref, Schema, Semaphore } from "effect"
...
const connect = (
  ...
  Effect.callback<MachineOutcomeValue, MachineProcessError>((resume) => {
    const decoder = new TextDecoder()
    const state: ConnectionState = { text: "", settled: false }
    let socket: Bun.Socket<ConnectionState> | undefined
    const complete = (effect: Effect.Effect<MachineOutcomeValue, MachineProcessError>) => {
      if (state.settled) return
      state.settled = true
      resume(effect)
    }
    void Bun.connect<ConnectionState>({
      unix: socketPath,
      socket: {
        open(opened) {
          socket = opened
    ...
    }).catch((error) => complete(Effect.fail(MachineProcessError.make({ message: String(error) }))))
    return Effect.sync(() => socket?.end())
  })
