...
import { Effect, Encoding, FileSystem, Function, Result, Schema, Stream } from "effect"
...
export const failure: {
  (kind: WorkspaceArchiveError["kind"], message: string): WorkspaceArchiveError
  (message: string): (kind: WorkspaceArchiveError["kind"]) => WorkspaceArchiveError
} = Function.dual(2, (kind: WorkspaceArchiveError["kind"], message: string) =>
  WorkspaceArchiveError.make({ kind, message }),
)
...
export const exclusionArguments = excluded.flatMap((path) => {
...
export const tarExecutable = Effect.fn("WorkspaceArchive.tarExecutable")(function* () {
...
export const command = Effect.fn("WorkspaceArchive.command")(function* (
...
export const archiveCompression = (bytes: Uint8Array): "gzip" | "zstd" | "none" => {
...
export const existingFiles = Effect.fn("WorkspaceArchive.existingFiles")(function* (
  workspace: string,
  listed: Uint8Array,
) {
...
export const inspectSecretFiles = Effect.fn("WorkspaceArchive.inspectSecretFiles")(function* (
  directory: string,
  listed: Uint8Array,
) {
  const fileSystem = yield* FileSystem.FileSystem
...
export const inspectGitSecretChanges = Effect.fn("WorkspaceArchive.inspectGitSecretChanges")(function* (
  workspace: string,
  files: Uint8Array,
) {
