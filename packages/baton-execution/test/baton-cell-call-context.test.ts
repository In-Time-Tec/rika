...
it.effect("answers each Session's binding request under that Session's own cell identity", () =>
  Effect.gen(function* () {
    ...
    const calls = Context.get(yield* Layer.build(CellCallContext.layer), CellCallContext.CellCallContext)
    const bound = CellCallContext.bindCalls(observingRegistry(observed), calls)
...
it.effect("carries the cell's nested-operation journal and Session store to the handler", () =>
  Effect.gen(function* () {
    ...
    const calls = Context.get(yield* Layer.build(CellCallContext.layer), CellCallContext.CellCallContext)
    const bound = CellCallContext.bindCalls(registry, calls)
