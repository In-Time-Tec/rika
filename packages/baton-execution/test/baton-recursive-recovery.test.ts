...
const waitForRecursiveSuspension = (filename: string) =>
  Effect.sync(() => {
    const database = new Database(filename, { readonly: true })
    const runs = database
      .query<
        { depth: number; status: string },
        []
      >("SELECT depth, status FROM baton_runs WHERE depth IN (1, 2) ORDER BY depth")
      .all()
    database.close()
    return runs
  }).pipe(
...
it.live(
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        const filename = `/tmp/rika-baton-recursive-recovery-${yield* Random.nextInt}.db`
        ...
        const link = yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* waitForRecursiveSuspension(filename)
            return admitted
          }),
        )
