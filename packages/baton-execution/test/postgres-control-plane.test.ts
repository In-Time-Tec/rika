...
import * as PostgresControlPlane from "../src/postgres-control-plane"
...
const options = {
  url: "postgresql://runtime.invalid/baton",
  source: "rika-baton",
  maxConnections: 10,
  worker,
} as const
...
it.effect("validates the complete PostgreSQL and worker configuration", () =>
  Effect.gen(function* () {
    expect(yield* PostgresControlPlane.validateOptions(options)).toEqual(options)
    expect(PostgresControlPlane.runtimeWorkerOptions(worker)).toEqual({
      workerId: "railway-worker-01",
      concurrency: 4,
      lease: 30_000,
      pollInterval: 200,
...
it.effect("rejects missing identities and non-positive PostgreSQL worker bounds before opening a database", () =>
  Effect.gen(function* () {
    ...
    for (const candidate of invalid) {
      const exit = yield* Effect.exit(PostgresControlPlane.validateOptions(candidate))
...
it.effect("composes the released RuntimeWorker loop without giving an executor storage services", () =>
  Effect.gen(function* () {
    ...
    const context = yield* Layer.build(PostgresControlPlane.workerLayer(worker).pipe(Layer.provide(dependencies)))
