...
export const makeUsageAccounting = (pricing: "included" | "metered" = "metered"): UsageAccounting => {
  ...
  const observeLifecycleAt = (event: RunEvent.RunEvent): number => {
    ...
    if (lastLifecycleAt !== undefined && at < lastLifecycleAt)
      throw new TypeError(`Baton lifecycle timestamp regressed: ${event.eventId}`)
    lastLifecycleAt = at
    return at
  }
  ...
  const activate = (node: Node, event: RunEvent.RunEvent) => {
    ...
    if (node.lifecycle === "active") throw new TypeError(`Baton Run ${node.rawRunId} activated twice`)
    if (node.lifecycle === "terminal") throw new TypeError(`Baton Run ${node.rawRunId} activated after terminal`)
  ...
  const deactivate = (node: Node, event: RunEvent.RunEvent, next: "waiting" | "terminal") => {
    ...
    if (node.lifecycle === "active") {
      if (activeDepth <= 0 || activeSince === undefined) throw new TypeError("Invalid Baton active-time depth")
      activeDepth -= 1
      if (activeDepth === 0) {
      ...
      throw new TypeError(`Baton Run ${node.rawRunId} waited while ${node.lifecycle}`)
    else if (next === "terminal" && node.lifecycle === "terminal")
      throw new TypeError(`Baton Run ${node.rawRunId} settled twice`)
    node.lifecycle = next
  }
  ...
  return {
    ...
    restore: (persisted) => {
      ...
      if (
        ...
        throw new TypeError("Invalid bounded Baton usage checkpoint")
    },
  }
}
