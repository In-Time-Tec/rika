...
type NativeRequest = Extract<ToolRuntime.Request, { _tag: "Bash" | "Read" | "Edit" }>
type ErrorRequest = NativeRequest | Extract<ToolRuntime.Request, { _tag: "ShellCommandStatus" }>

export interface RunnerNativeRuntimeService {
  readonly run: (request: NativeRequest) => Effect.Effect<Result, ToolRuntime.ToolError>
  readonly observeProcess: ToolRuntime.Interface["observeProcess"]
}

export class RunnerNativeRuntime extends Context.Service<RunnerNativeRuntime, RunnerNativeRuntimeService>()(
  "@rika/runner/native/runtime/RunnerNativeRuntime",
) {}
...
const toolName = (request: ErrorRequest) => request._tag.replaceAll(/([a-z])([A-Z])/g, "$1_$2").toLowerCase()
const contract = (request: ErrorRequest): ToolPolicy.Policy => policyForName(toolName(request))!
...
const toolError = (request: ErrorRequest, cause: unknown, kind: "operation" | "timeout") => {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const startChecked = (
        ...
        cwd: string,
      ): Effect.Effect<string, RuntimeOperationError | PlatformError.PlatformError> => {
      ...
      const bash = Effect.fn("NativeToolRuntime.bash")(function* (
        ...
        const output = yield* Effect.acquireUseRelease(
          startChecked("/bin/bash", ["-lc", request.command], cwd),
          (processId) =>
            Effect.gen(function* () {
              ...
              yield* processes.observe(processId)
              const terminal = yield* processes.poll(processId, 0, contract(request).outputLimit)
              return {
                ...terminal,
                stdout: `${first.stdout}${terminal.stdout}`,
                stderr: `${first.stderr}${terminal.stderr}`,
                truncated: first.truncated || terminal.truncated,
              }
            }),
          (processId, exit) => {
            const cleanup = processes.cancel(processId)
            return Exit.isFailure(exit) && Cause.hasInterrupts(exit.cause) ? cleanup.pipe(Effect.ignore) : cleanup
          },
        )
