...
import { sameBinding, sameWorkspacePolicy } from "@rika/execution"
import {
  ActivatedSkill,
  Authorization,
  ChildAttenuation,
...
export interface ContextMaterializerService {
  ...
  ) => Effect.Effect<SessionMaterialization, ContextMaterializationError>
  readonly restore: (
...
export class SessionAuthorization extends Context.Service<SessionAuthorization, SessionAuthorizationService>()(
  "@rika/context/materializer/SessionAuthorization",
) {}
...
const discoverWithReader = (reader: WorkspaceReaderService, input: DiscoverInput) =>
  Effect.gen(function* () {
    ...
    const discoveredSkills = yield* reader
      .listSkills(binding)
      .pipe(Effect.mapError((error) => asError(error, "reader")))
    const limits = {
      maxSkills: Math.max(0, Math.floor(input.limits?.maxSkills ?? defaultLimits.maxSkills)),
      maxDescriptionChars: Math.max(
        1,
        Math.floor(input.limits?.maxDescriptionChars ?? defaultLimits.maxDescriptionChars),
      ),
      maxSkillListingTokens: Math.max(
        0,
        Math.floor(input.limits?.maxSkillListingTokens ?? defaultLimits.maxSkillListingTokens),
      ),
    }
...
const activateWithReader = (reader: WorkspaceReaderService, input: ActivateSkillInput) =>
  Effect.gen(function* () {
    ...
    const body = yield* skill.instructions.pipe(Effect.mapError((error) => failure("skill-body", error.message)))
    ...
    const activatedSkills = [
      ...input.materialization.activatedSkills.filter((item) => item.name !== input.name),
      activated,
    ]
    return Schema.decodeUnknownSync(SessionMaterialization)({
...
const pinnedGuidance = (materialization: SessionMaterialization) =>
  Effect.gen(function* () {
    const decoded = yield* Instructions.Snapshot.decode(
      materialization.registration.id,
      materialization.registration.payload,
    ).pipe(Effect.mapError(() => failure("content-missing", "Pinned guidance content cannot be reconstructed")))
...
export const effectiveAuthorization = (materialization: SessionMaterialization, authorization: Authorization) =>
  Effect.sync(() => {
    ...
    const allowedModels = authorization.allowedModels.includes(materialization.modelPin)
      ? [materialization.modelPin]
      : []
...
const attenuateChild = (materialization: SessionMaterialization, requested: ChildAttenuation) =>
  Effect.gen(function* () {
    ...
    if (
      requested.allowedCredentials.some(
        (credential) => !materialization.model.credentialRefs.some((parent) => sameCredential(parent, credential)),
      )
    )
...
const sameCredential = (
  left: { readonly provider: string; readonly reference: string },
  right: { readonly provider: string; readonly reference: string },
  ...
  Effect.gen(function* () {
    ...
    if (!current.canResume)
      return yield* Effect.fail(
        failure("revoked", "Child execution requires the current model and credentials to remain authorized"),
      )
    const narrowed = yield* attenuateChild(materialization, requested)
    if (
      materialization.model.credentialRefs.some(
        (required) => !requested.allowedCredentials.some((allowed) => sameCredential(allowed, required)),
      )
    )
      return yield* Effect.fail(
        failure("attenuation", "Child credentials must include every credential required by the selected model"),
      )
    ...
    if (
      requested.allowedCredentials.some(
        (credential) => !current.allowedCredentials.some((allowed) => sameCredential(allowed, credential)),
      )
    )
      return yield* Effect.fail(
        failure("revoked", "Child credentials must remain granted by the current authorization"),
      )
    return narrowed
  })
      ...
      return ChildAdmission.AgentChildren.of({
        admit: (input) =>
          Effect.gen(function* () {
            ...
            const materialization = yield* Components.read(options.declaration).pipe(
            ...
            yield* authorizeChildWithAuthorization(materialization, current, requested).pipe(
              Effect.mapError(runtimeUnavailable),
            )
...
export const composeSessionContext = (agent: Agent.Any, materialization: SessionMaterialization) =>
  Effect.gen(function* () {
    ...
    const instructions =
      agent.instructions === undefined ? contextInstructions : `${agent.instructions}\n\n${contextInstructions}`
    ...
    return {
      ...
      instructions: Instructions.layer([
        {
          id: `rika/context-v2/${agent.name}/${materialization.guidance.id}`,
          render: (context) =>
            Effect.succeed(context.agentName === agent.name ? Option.some(instructions) : Option.none()),
        },
      ]),
      model,
...
const componentTransition = (
  ...
  if (command._tag === "RebindWorkspace") {
    if (!sameBinding(state.workspace, command.expected)) throw new Error("Workspace binding conflict")
    if (
      !sameWorkspacePolicy(state.workspace, command.workspace) ||
      command.workspace.generation <= state.workspace.generation
    )
      throw new Error("Workspace recovery must retain its policy and advance its generation")
    return { ...state, workspace: command.workspace }
  }
  if (command._tag === "ActivateSkill") {
    const skill = metadataByName(state, command.name)
    if (skill === undefined || skill.disableModelInvocation === true)
      throw new Error("Selected skill is not activatable")
    if (
...
export class ContextMaterializer extends Context.Service<ContextMaterializer, ContextMaterializerService>()(
  "@rika/context/materializer/ContextMaterializer",
) {}
