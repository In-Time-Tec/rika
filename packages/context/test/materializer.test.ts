...
import { Agent, Approvals, Instructions, ModelRegistry, Permissions, SkillCatalog, ToolContext } from "generalist"
...
it.effect("captures workspace guidance from the explicit binding and retains it after changes", () =>
  Effect.gen(function* () {
    ...
    expect(initial.guidance.payload.entries.find((entry) => entry.path === "AGENTS.md")?.content).toBe(
      "initial guidance",
    )
  }),
)
...
it.effect("fails closed when pinned guidance or activated content is missing", () =>
  Effect.gen(function* () {
    ...
    const missing = {
      ...activated,
      registration: { ...activated.registration, payload: { ...activated.registration.payload, entries: [] } },
    }
    ...
    const compositionError = yield* Effect.flip(
      composeSessionContext(Agent.make({ name: "context", input: Schema.String, output: Schema.String }), missing),
    )
    expect(compositionError.reason).toBe("content-missing")
  }),
)

it.effect("preserves the Agent baseline in rendered workspace instructions", () =>
  Effect.gen(function* () {
    const materialization = yield* discoverWith([])
    const baseline = "Keep Rika native operations inside the assigned workspace."
    const composition = yield* composeSessionContext(
      Agent.make({ name: "rika", input: Schema.String, output: Schema.String, instructions: baseline }),
      materialization,
    )
    const instructions = yield* Instructions.Instructions.pipe(Effect.provide(composition.instructions))
    const rendered = yield* Instructions.render(instructions, { agentName: "rika", turn: 0 })
    expect(rendered).toContain(baseline)
    expect(rendered.split("initial guidance")).toHaveLength(2)
    expect(rendered.split(baseline)).toHaveLength(2)
    expect(yield* Instructions.render(instructions, { agentName: "another-agent", turn: 0 })).toBe("")
  }),
)
...
it.effect("records settings changes through the canonical component transition and attenuates children", () =>
  Effect.gen(function* () {
    ...
    const child = yield* attenuate(changed, {
      allowedTools: [],
    ...
    const childError = yield* Effect.flip(
      attenuate(changed, { allowedTools: ["ungranted"], allowedModels: [], allowedCredentials: [] }),
    ).pipe(
      Effect.provide(
        contextLayer.pipe(
          Layer.provide(layerTest({ readGuidance: () => Effect.succeed([]), listSkills: () => Effect.succeed([]) })),
...
const configuredModelLayer = (
  fixture: TestModel.Fixture,
  configuration: ModelConfiguration,
  seen: Array<ModelConfiguration>,
) => {
...
const runSettings = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const selectedModelLayer = yield* modelLayer(initial, (selected) =>
        configuredModelLayer(fixture, selected, modelCalls),
      )
...
it.effect("persists Session components across fresh Runtime layers and deduplicates command receipts", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const changedModel: ModelConfiguration = {
        ...runtimeModel,
        settings: { ...runtimeModel.settings, reasoningEffort: "high" },
      }
      const command = ChangeSettingsCommand.make({
      ...
      expect(first.modelCalls[0]?.settings).toMatchObject({
        temperature: 0.25,
        maxOutputTokens: 128,
        reasoningEffort: "low",
      })
      ...
      const restored = yield* Schema.decodeUnknownEffect(SessionMaterialization)(
        first.execution.sessionComponents?.[0]?.state,
      )
      ...
      expect(retry.modelCalls[0]?.settings).toMatchObject({
        temperature: 0.25,
        maxOutputTokens: 128,
...
it.effect("captures model input in the durable Run request", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      expect(result.modelCalls[0]?.settings).toMatchObject({
        temperature: 0.25,
        maxOutputTokens: 128,
        reasoningEffort: "low",
      })
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        const host = yield* Host.make({
          agents: { parent: parent.agent, child: child.agent },
          revision: "context-v2-child-test",
        })
        const activation = yield* activate
        yield* Effect.gen(function* () {
          ...
          yield* authorizeChild(materialization, authorization, request).pipe(
            Effect.provide(
              contextLayer.pipe(
                Layer.provide(
                  layerTest({ readGuidance: () => Effect.succeed([]), listSkills: () => Effect.succeed([]) }),
                ),
              ),
            ),
          )
          ...
          const denied = yield* Effect.flip(
            authorizeChild(materialization, { ...authorization, allowedCredentials: [] }, request).pipe(
              Effect.provide(
                contextLayer.pipe(
                  Layer.provide(
                    layerTest({ readGuidance: () => Effect.succeed([]), listSkills: () => Effect.succeed([]) }),
          ...
          const deniedModel = yield* Effect.flip(
            authorizeChild(materialization, { ...authorization, allowedModels: [] }, request).pipe(
              Effect.provide(
                contextLayer.pipe(
                  Layer.provide(
                    layerTest({ readGuidance: () => Effect.succeed([]), listSkills: () => Effect.succeed([]) }),
...
it.effect("guards model-owned child admission through the Runtime AgentChildren service", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const modelCalls: Array<ModelConfiguration> = []
      const selectedModelLayer = yield* modelLayer(materialization, (selected) =>
        configuredModelLayer(fixture, selected, modelCalls),
      )
      const authorizationLayer = Layer.succeed(
        SessionAuthorization,
        SessionAuthorization.of({ current: () => Ref.get(authorizationRef) }),
      )
