...
export const DispatchPhase = Schema.Literals(["before-dispatch", "after-dispatch", "reconnect"])
...
export interface ExecutorBoundary {
  /** Prove workspace, assignment generation, build and protocol before every effect. */
  readonly handshake: (request: HandshakeRequest) => Effect.Effect<HandshakeEvidence, ExecutorTransportError>
  /** Dispatch exactly one admitted identity. The boundary must not synthesize a different operation identity. */
  readonly dispatch: (
    intent: NativeOperationIntent,
    handshake: HandshakeEvidence,
  ) => Effect.Effect<ExecutorEvidence, ExecutorTransportError>
  /** Read executor evidence after reconnect; this is never accepted without canonical validation. */
  readonly receipt: (operationId: string) => Effect.Effect<ExecutorEvidence | undefined, ExecutorTransportError>
}

export class Executor extends Context.Service<Executor, ExecutorBoundary>()("@rika/execution-v2/executor") {}
