  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const get = Effect.fn("ThreadRepository.get")(function* (id: ThreadId) {
        const rows = yield* sql`SELECT * FROM rika_threads WHERE id = ${id} AND owner_id = ${ownerId}
      ...
      return Service.of({
          ...
          return yield* sql
            .withTransaction(
              Effect.gen(function* () {
                ...
                WHERE id = ${currentId} AND owner_id = ${ownerId}`.pipe(Effect.mapError(repositoryError))
                return created
              }),
            )
            .pipe(Effect.mapError(repositoryError))
        ...
        listAll: Effect.gen(function* () {
          const rows = yield* sql`SELECT * FROM rika_threads WHERE owner_id = ${ownerId}
            AND NOT EXISTS (SELECT 1 FROM rika_thread_deletion_outbox WHERE thread_id = rika_threads.id)`.pipe(
            Effect.mapError(repositoryError),
          )
