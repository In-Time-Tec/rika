    ...
    Effect.gen(function* () {
      ...
      const update = Effect.fn("ThreadRepository.update")(function* (
        ...
        if (fields.labels !== undefined)
          values.labelsJson = yield* Schema.encodeEffect(LabelsJson)(fields.labels).pipe(
      ...
      const insert = Effect.fn("ThreadRepository.insert")(function* (input: CreateInput) {
        yield* db
          ...
          .values({ ownerId, path: input.workspace, createdAt: input.now })
          .onConflictDoNothing()
          .pipe(Effect.mapError(repositoryError))
      ...
      return Service.of({
          ...
          return yield* db
            .transaction((tx) =>
              Effect.gen(function* () {
                ...
                yield* tx
                  .update(rikaThreads)
                  .set({ archived: 1, updatedAt: input.now })
                  .where(and(eq(rikaThreads.id, currentId), eq(rikaThreads.ownerId, ownerId)))
                  .pipe(Effect.mapError(repositoryError))
                return created
              }),
            )
        ...
        list: Effect.fn("ThreadRepository.list")(function* (input = {}) {
          ...
          if (input.query !== undefined)
            filters.push(
              or(
                ilike(rikaThreads.title, `%${input.query}%`),
                ilike(rikaThreads.workspace, `%${input.query}%`),
        ...
        requestDeletion: Effect.fn("ThreadRepository.requestDeletion")(function* (id, requestedAt) {
          ...
          yield* db
            .insert(rikaThreadDeletionOutbox)
            .values({ threadId: id, requestedAt })
            .onConflictDoNothing()
            .pipe(Effect.mapError(repositoryError))
        }),
        pendingDeletions: Effect.gen(function* () {
          const rows = yield* db
            .select({ threadId: rikaThreadDeletionOutbox.threadId, requestedAt: rikaThreadDeletionOutbox.requestedAt })
            .from(rikaThreadDeletionOutbox)
            .innerJoin(rikaThreads, eq(rikaThreads.id, rikaThreadDeletionOutbox.threadId))
            .where(eq(rikaThreads.ownerId, ownerId))
            .orderBy(asc(rikaThreadDeletionOutbox.requestedAt), asc(rikaThreadDeletionOutbox.threadId))
            .pipe(Effect.mapError(repositoryError))
