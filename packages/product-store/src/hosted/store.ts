...
import { requireActiveClient, requireThreadAccess } from "./authority"
import {
...
const allocateCommitCursor = Effect.fn("HostedStore.allocateCommitCursor")(function* (
  db: DatabaseExecutor,
  ownerId: string,
) {
...
const requireOwnerCreator = Effect.fn("HostedStore.requireOwnerCreator")(function* (
  db: DatabaseExecutor,
...
const requireOrganizationGrantAuthority = Effect.fn("HostedStore.requireOrganizationGrantAuthority")(function* (
  db: DatabaseExecutor,
...
const make = Effect.gen(function* (): Effect.fn.Return<StoreService, never, PgClient.PgClient> {
  ...
  const putOwner = Effect.fn("HostedStore.putOwner")(function* (input: PutOwnerInput) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const createProject = Effect.fn("HostedStore.createProject")(function* (input: CreateProjectInput) {
  ...
  const putProjectGrant = Effect.fn("HostedStore.putProjectGrant")(function* (input: PutProjectGrantInput) {
  ...
  const createWorkspace = Effect.fn("HostedStore.createWorkspace")(function* (input: CreateWorkspaceInput) {
  ...
  const createThread = Effect.fn("HostedStore.createThread")(function* (input: CreateThreadInput) {
    if (input.executorKind === "runner" && input.inheritProjectGrants === true)
  ...
  const readThread = Effect.fn("HostedStore.readThread")(function* (input: ReadThreadInput) {
    const rows = yield* query(
      db
        .select({
          id: rikaHostedThreads.id,
  ...
  const putThreadGrant = Effect.fn("HostedStore.putThreadGrant")(function* (input: PutThreadGrantInput) {
    yield* requireOrganizationGrantAuthority(db, {
      ownerId: input.ownerId,
      userId: input.grantedByUserId,
      membershipId: input.membershipId,
  ...
  const registerDevice = Effect.fn("HostedStore.registerDevice")(function* (input: RegisterDeviceInput) {
    const rows = yield* query(
      db
  ...
  const authenticateClient = Effect.fn("HostedStore.authenticateClient")(function* (input: AuthenticateClientInput) {
  ...
  const validateClient: StoreService["validateClient"] = Effect.fn("HostedStore.validateClient")(function* (input) {
    const rows = yield* query(
      db
        .select({ present: expression<number>`1` })
  ...
  const grantClientAuthority: StoreService["grantClientAuthority"] = Effect.fn("HostedStore.grantClientAuthority")(
    function* (input) {
      const now = timestamp(input.now)
      const expiresAt = timestamp(input.expiresAt)
  ...
  const authorizeThread: StoreService["authorizeThread"] = Effect.fn("HostedStore.authorizeThread")((input) =>
    db
      .transaction((tx) => requireThreadAccess(tx, input, input.action, input.at))
      .pipe(Effect.catchTag("SqlError", databaseError)),
  )

  const admitCommand = Effect.fn("HostedStore.admitCommand")(function* (input: AdmitCommandInput) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const readCommands: StoreService["readCommands"] = Effect.fn("HostedStore.readCommands")(function* (input) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const appendEvent = Effect.fn("HostedStore.appendEvent")(function* (input: AppendEventInput) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const appendRecoveredEvent = Effect.fn("HostedStore.appendRecoveredEvent")(function* (
  ...
  const readEvents: StoreService["readEvents"] = Effect.fn("HostedStore.readEvents")(function* (input) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
          yield* requireThreadAccess(tx, input, "thread:view")
  ...
  const acknowledgeCursor: StoreService["acknowledgeCursor"] = Effect.fn("HostedStore.acknowledgeCursor")(
    function* (input) {
      return yield* db
        .transaction((tx) =>
          Effect.gen(function* () {
  ...
  const acquireTerminalWriter = Effect.fn("HostedStore.acquireTerminalWriter")(function* (
  ...
  const upsertPresence = Effect.fn("HostedStore.upsertPresence")(function* (input: UpsertPresenceInput) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const listPresence: StoreService["listPresence"] = Effect.fn("HostedStore.listPresence")(function* (input) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const recordAuditEvent = Effect.fn("HostedStore.recordAuditEvent")(function* (input: RecordAuditEventInput) {
    return yield* db
      .transaction((tx) =>
        Effect.gen(function* () {
  ...
  const putCredentialReference = Effect.fn("HostedStore.putCredentialReference")(function* (
