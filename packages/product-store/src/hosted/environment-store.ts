...
const reference = (row: EnvironmentRow): Effect.Effect<EnvironmentReferenceValue, EnvironmentStoreError> =>
  Schema.decodeUnknownEffect(EnvironmentReference)(row.projectId === null ? {
    id: row.id,
    ownerId: row.ownerId,
    scope: row.scope,
    scopeId: row.scopeId,
...
const stored = Effect.fn("EnvironmentStore.stored")(function* (
  ...
  if (!(row.nonce instanceof Uint8Array) || !(row.ciphertext instanceof Uint8Array) ||
...
const make = Effect.gen(function* (): Effect.fn.Return<EnvironmentStoreService, never, PgClient.PgClient> {
  ...
  const putValue: EnvironmentStoreService["putValue"] = Effect.fn("EnvironmentStore.putValue")(
    function* (input) {
      ...
      return yield* db.transaction((tx) => Effect.gen(function* () {
        const rows = yield* query(tx.insert(rikaHostedEnvironmentValues).values({
          id: input.id,
        ...
        const selected = (yield* query(tx.select().from(rikaHostedEnvironmentValues).where(and(
  ...
  const revokeValue: EnvironmentStoreService["revokeValue"] = Effect.fn("EnvironmentStore.revokeValue")(
    function* (input) {
      return yield* db.transaction((tx) => Effect.gen(function* () {
        ...
        const rows = yield* query(tx.update(rikaHostedEnvironmentValues).set({
          state: "revoked",
          revision: expression`${rikaHostedEnvironmentValues.revision} + 1`,
          keyVersion: null,
          nonce: null,
        ...
        }).where(predicate).returning({ id: rikaHostedEnvironmentValues.id }))
  ...
  const putOrganizationPolicy: EnvironmentStoreService["putOrganizationPolicy"] = Effect.fn(
    ...
    yield* query(db.insert(rikaHostedOrganizationEnvironmentPolicy).values({
      ownerId: input.ownerId,
  ...
  const putApproval: EnvironmentStoreService["putApproval"] = Effect.fn("EnvironmentStore.putApproval")(
    function* (input) {
      return yield* db.transaction((tx) => Effect.gen(function* () {
        ...
        yield* query(tx.insert(rikaHostedSourceEnvironmentApprovals).values({
          ownerId: input.ownerId,
          projectId: input.projectId ?? null,
        ...
        const selected = (yield* query(tx.select().from(rikaHostedSourceEnvironmentApprovals).where(approvalPredicate(input))))[0]
        if (selected === undefined) return yield* database()
  ...
  const revokeApproval: EnvironmentStoreService["revokeApproval"] = Effect.fn(
    "EnvironmentStore.revokeApproval",
  )(function* (input) {
    return yield* db.transaction((tx) => Effect.gen(function* () {
      const rows = yield* query(tx.update(rikaHostedSourceEnvironmentApprovals)
  ...
  const putEgress: EnvironmentStoreService["putEgress"] = Effect.fn("EnvironmentStore.putEgress")(
    function* (input) {
      return yield* db.transaction((tx) => Effect.gen(function* () {
        const predicate = and(
        ...
        yield* query(tx.insert(rikaHostedPhaseEgressPolicy).values({
          ownerId: input.ownerId,
          projectId: input.projectId ?? null,
  ...
  const resolvePhase: EnvironmentStoreService["resolvePhase"] = Effect.fn("EnvironmentStore.resolvePhase")(
    function* (input) {
      const rows = yield* query(db.select().from(rikaHostedEnvironmentValues).where(and(
        eq(rikaHostedEnvironmentValues.ownerId, input.ownerId),
        or(
          and(eq(rikaHostedEnvironmentValues.scope, "personal"), eq(rikaHostedEnvironmentValues.scopeId, input.userId)),
          and(eq(rikaHostedEnvironmentValues.scope, "organization"), eq(rikaHostedEnvironmentValues.scopeId, input.ownerId)),
          input.projectId === undefined ? undefined : and(
      ...
      const approvals = yield* query(db.select().from(rikaHostedSourceEnvironmentApprovals).where(and(
        eq(rikaHostedSourceEnvironmentApprovals.ownerId, input.ownerId),
        or(
          input.projectId === undefined ? undefined : eq(rikaHostedSourceEnvironmentApprovals.projectId, input.projectId),
        ...
        eq(rikaHostedSourceEnvironmentApprovals.phase, input.phase),
      )).orderBy(expression`${rikaHostedSourceEnvironmentApprovals.projectId} asc nulls last`).limit(1).for("share"))
      const policies = yield* query(db.select({
        personalOverrides: rikaHostedOrganizationEnvironmentPolicy.personalOverrides,
      }).from(rikaHostedOrganizationEnvironmentPolicy).where(
        eq(rikaHostedOrganizationEnvironmentPolicy.ownerId, input.ownerId),
      ).for("share"))
      const egressRows = yield* query(db.select({ allow: rikaHostedPhaseEgressPolicy.allowlist })
        .from(rikaHostedPhaseEgressPolicy).where(and(
          eq(rikaHostedPhaseEgressPolicy.ownerId, input.ownerId),
        ...
        )).orderBy(expression`${rikaHostedPhaseEgressPolicy.projectId} asc nulls last`).limit(1).for("share"))
      const resolved = {
  ...
  return EnvironmentStore.of({ putValue, revokeValue, putOrganizationPolicy, putApproval, revokeApproval, putEgress, resolvePhase })
})
