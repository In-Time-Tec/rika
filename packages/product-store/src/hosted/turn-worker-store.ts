...
const claim = (
  ...
  transaction(db, (tx) =>
    Effect.gen(function* () {
      ...
      const preparedExecution = row.preparedTurnJson === null ? undefined
        : yield* Schema.decodeEffect(PreparedTurnJson)(row.preparedTurnJson).pipe(Effect.mapError(failure))
      const admissionLink = row.admissionLinkJson === null ? undefined
  ...
  Effect.gen(function* () {
      ...
      claim(
        ...
        (tx) => {
          ...
          const activeClaim = tx
            ...
            .where(and(eq(claimedTurn.threadId, rikaTurns.threadId), gt(rikaHostedTurnClaims.expiresAt,
                    ...
                    tx
                      ...
                      .where(
                        and(
                          ...
                          gt(rikaHostedTurnClaims.expiresAt,
                            sql`floor(extract(epoch from transaction_timestamp()) * 1000)`),
                        ),
                      ),
                  ),
    ...
    const renew: HostedTurnWorkerStoreService["renew"] = Effect.fn("HostedTurnWorkerStore.renew")(
      function* (turnClaim, leaseMillis) {
        const databaseNow = sql<number>`floor(extract(epoch from transaction_timestamp()) * 1000)::bigint`
        const renewed = yield* query(db.update(rikaHostedTurnClaims).set({ heartbeatAt: databaseNow,
          expiresAt: sql`${databaseNow} + ${leaseMillis}` }).where(and(
          eq(rikaHostedTurnClaims.turnId, turnClaim.input.turnId), eq(rikaHostedTurnClaims.workerId, turnClaim.workerId),
          eq(rikaHostedTurnClaims.claimToken, turnClaim.claimToken), gt(rikaHostedTurnClaims.expiresAt, databaseNow),
      ...
      function* (turnClaim, prepared, now) {
        ...
        return yield* transaction(db, (tx) =>
          Effect.gen(function* () {
            ...
            const persisted = (yield* query(tx.select({ startInputJson: rikaTurnAdmissionOutbox.startInputJson,
              preparedTurnJson: rikaTurnAdmissionOutbox.preparedTurnJson }).from(rikaTurnAdmissionOutbox)
              .where(eq(rikaTurnAdmissionOutbox.turnId, turnClaim.input.turnId))))[0]
            ...
            const persistedInput = yield* Schema.decodeEffect(StartTurnJson)(persisted.startInputJson).pipe(Effect.mapError(failure))
            const persistedPrepared = yield* Schema.decodeEffect(PreparedTurnJson)(persisted.preparedTurnJson).pipe(Effect.mapError(failure))
    ...
    const completeAdmission: HostedTurnWorkerStoreService["completeAdmission"] = Effect.fn("HostedTurnWorkerStore.completeAdmission")(
    ...
    const requestActivation: HostedTurnWorkerStoreService["requestActivation"] = Effect.fn("HostedTurnWorkerStore.requestActivation")(
      function* (turnClaim, now) {
        return yield* transaction(db, (tx) => Effect.gen(function* () {
          const authority = yield* query(tx.select({ turnId: rikaHostedTurnClaims.turnId }).from(rikaHostedTurnClaims)
            .where(and(eq(rikaHostedTurnClaims.turnId, turnClaim.input.turnId),
              eq(rikaHostedTurnClaims.workerId, turnClaim.workerId), eq(rikaHostedTurnClaims.claimToken, turnClaim.claimToken),
              gt(rikaHostedTurnClaims.expiresAt, sql`floor(extract(epoch from transaction_timestamp()) * 1000)`))).for("update"))
          if (authority[0] === undefined) return yield* failure("Turn claim is no longer owned by this worker")
          const admission = (yield* query(tx.select({ link: rikaTurnAdmissionOutbox.admissionLinkJson,
            requestedAt: rikaTurnAdmissionOutbox.activationRequestedAt }).from(rikaTurnAdmissionOutbox)
            .where(eq(rikaTurnAdmissionOutbox.turnId, turnClaim.input.turnId)).for("update")))[0]
          if (admission?.link === null || admission === undefined) return yield* failure("Turn has no staged Runtime admission")
          const turn = (yield* query(tx.select({ status: rikaTurns.status, executionLinkJson: rikaTurns.executionLinkJson })
            .from(rikaTurns).where(and(eq(rikaTurns.id, turnClaim.input.turnId),
              eq(rikaTurns.threadId, turnClaim.input.threadId))).for("update")))[0]
          ...
          const activated = yield* query(tx.update(rikaTurns).set({ executionLinkJson: admission.link, updatedAt: now })
          ...
          yield* query(tx.update(rikaTurnAdmissionOutbox).set({ activationRequestedAt: now })
            .where(eq(rikaTurnAdmissionOutbox.turnId, turnClaim.input.turnId)))
          return true
        }))
      },
    )
    const completeActivation: HostedTurnWorkerStoreService["completeActivation"] = Effect.fn("HostedTurnWorkerStore.completeActivation")(
      function* (turnClaim, status, now) {
        yield* transaction(db, (tx) => Effect.gen(function* () {
          const authority = yield* query(tx.select({ turnId: rikaHostedTurnClaims.turnId }).from(rikaHostedTurnClaims)
            .where(and(eq(rikaHostedTurnClaims.turnId, turnClaim.input.turnId),
              eq(rikaHostedTurnClaims.workerId, turnClaim.workerId), eq(rikaHostedTurnClaims.claimToken, turnClaim.claimToken),
              gt(rikaHostedTurnClaims.expiresAt, sql`floor(extract(epoch from transaction_timestamp()) * 1000)`))).for("update"))
          if (authority[0] === undefined) return yield* failure("Turn claim is no longer owned by this worker")
          yield* query(tx.update(rikaTurns).set({ status, updatedAt: now }).where(and(
            eq(rikaTurns.id, turnClaim.input.turnId), eq(rikaTurns.status, "accepted"))))
          yield* query(tx.delete(rikaTurnAdmissionOutbox).where(eq(rikaTurnAdmissionOutbox.turnId, turnClaim.input.turnId)))
          yield* query(tx.delete(rikaHostedTurnClaims).where(and(eq(rikaHostedTurnClaims.turnId, turnClaim.input.turnId),
            eq(rikaHostedTurnClaims.workerId, turnClaim.workerId), eq(rikaHostedTurnClaims.claimToken, turnClaim.claimToken))))
        }))
      },
    )
