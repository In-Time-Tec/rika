...
export type ProductRecord = typeof JsonObject.Type
...
const statusError = (response: Response) => {
  ...
  return ProductClientError.make({ kind, message: `Product request failed (${response.status})`, status: response.status })
}
...
const makeRequest = (input: {
  ...
  const request = (method: string, path: string, body?: ProductRecord, headers?: HeaderInput) => {
  ...
  return <A, S extends Schema.Schema<A>>(method: string, path: string, schema: S, body?: ProductRecord, headers?: HeaderInput) =>
    request(method, path, body, headers).pipe(
      Effect.flatMap((response) => (response.ok ? decodeJson(schema, response, "Product response was invalid") : Effect.fail(statusError(response)))),
    )
}
...
export const makeProductClient = (options: {
  ...
  return {
    ...
    listThreads: (input = {}) => {
      ...
      return request("GET", `${paths.threads}${suffix}`, ThreadPage)
    },
