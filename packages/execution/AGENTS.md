...
The API alone receives PostgreSQL, RunStore, claims, RuntimeWorker, and agent-loop authority. Runner and Orb executors receive no database services; they execute workspace-bound cells through `RemoteCells`, deduplicated by Generalist's stable operation key. In-memory Runtime layers exist only for deterministic tests.
