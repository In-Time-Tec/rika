...
const bindTransition = (state: WorkspaceComponentState, binding: WorkspaceBinding): WorkspaceComponentState => {
  ...
  if (!sameWorkspacePolicy(state.binding, binding)) throw new Error("Workspace recovery cannot change its policy")
  ...
  if (state.admitted.length !== 0) throw new Error("Workspace recovery cannot discard unresolved native operations")
  return { binding, admitted: state.admitted }
}
...
const admitTransition = (state: WorkspaceComponentState, operation: ComponentOperation): WorkspaceComponentState => {
...
const readParentComponent = (toolContext: ToolContext.Service, runStore: RunStore.Service) =>
  Effect.gen(function* () {
    ...
    if (toolRunId === undefined) return yield* componentFailure("unavailable")
    const toolExecution = yield* runStore.loadExecution(toolRunId)
    const parentRunId = toolExecution.parentRunId
    if (parentRunId === undefined) return yield* componentFailure("unavailable")
    const parentExecution = yield* runStore.loadExecution(parentRunId)
    if (parentExecution.rootRunId !== toolExecution.rootRunId) return yield* componentFailure("unavailable")
...
export const nativeOperationAuthority = (input: {
  readonly context: ToolContext.Service
  readonly store: RunStore.Service
}): WorkspaceComponentJournal => ({
  ...workspaceComponentReader(input.context, input.store),
  read: Effect.gen(function* () {
    const { context, store } = input
    if (context.runId === undefined || context.operationKey === undefined) return yield* componentFailure("unavailable")
    const execution = yield* store.loadExecution(context.runId)
    const executable = execution.executableManifest.entries.find(
      (entry) => entry.pin === execution.executableRef.active,
    )
    if (executable?._tag !== "Tool") return yield* componentFailure("rejected")
    if (execution.parentRunId === undefined) return yield* componentFailure("rejected")
    const parentOperation = yield* store.getOperationByKey({
      runId: execution.parentRunId,
      operationKey: execution.message.idempotencyKey,
    })
    if (parentOperation?.kind !== "tool") return yield* componentFailure("rejected")
    const parentInput = yield* Schema.decodeUnknownEffect(Schema.Struct({ name: Schema.String }))(parentOperation.input)
    const operation = yield* store.getOperationByKey({ runId: context.runId, operationKey: context.operationKey })
    if (operation?.kind !== "tool" || operation.status !== "running") return yield* componentFailure("rejected")
    const state = yield* readParentComponent(context, store)
    if (state.binding === null) return yield* componentFailure("rejected")
    return {
      binding: state.binding,
      admitted: [
        {
          operationId: operation.operationKey,
          tool: parentInput.name,
          inputDigest: operation.inputDigest,
          binding: state.binding,
        },
      ],
