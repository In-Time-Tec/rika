...
test("100 concurrent queue submissions produce contiguous revisions and one coalesced wake", () => {
  const program = Effect.scoped(
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem
      ...
      yield* Effect.gen(function* () {
        ...
        expect(queue).toMatchObject({ revision: 100, queuedCount: 100 })
        expect(queue.turns).toHaveLength(100)
        const wake = yield* turns.requestQueueWake(id)
        expect(wake).toEqual({ threadId: id, generation: 1, queueRevision: 100 })
        ...
        expect(claimed?.queue).toMatchObject({ revision: 101, queuedCount: 99 })
        const sql = yield* SqlClient
