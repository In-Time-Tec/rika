    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          expect(migrationRows.at(-1)).toEqual({ migration_id: 22, name: "reconciled_child_trees" })
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          expect(yield* sql`SELECT COUNT(*) AS count FROM rika_migrations`).toEqual([{ count: 22 }])
