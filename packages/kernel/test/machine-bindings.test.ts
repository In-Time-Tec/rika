...
describe("machine bindings", () => {
  ...
  it.effect("turns an unobservable machine outcome into a known coding-tool failure", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const context = yield* Layer.build(
          MachineBindings.layer({ execute: () => Effect.succeed({ _tag: "Unknown", message: "lost" }) }),
        )
        const runtime = Context.get(context, CodingToolRuntime.Service)
        const result = yield* Effect.result(runtime.run({ _tag: "Bash", command: "ls" }))
        expect(result._tag).toBe("Failure")
        if (result._tag === "Failure")
          expect(result.failure).toMatchObject({
            _tag: "ToolError",
            tool: "bash",
            message: "machine coding-tool outcome is not safely observable",
            kind: "operation",
            outcome: "unknown",
            recovery: "later",
          })
