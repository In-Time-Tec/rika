...
export const cellInstructions = (options: Options): string =>
  [
    ...
    "",
    "A harness write takes the baseSnapshot from a fresh rika.harness.snapshot({ scope }) in the same",
    "cell. A stale one is refused, because it is what makes a concurrent write observable.",
    "",
    "// available on rika:",
    surface(options),
  ].join("\n")
