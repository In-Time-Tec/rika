...
test("keeps representative terminal workloads within CI performance envelopes", async () => {
  const result = await runTerminalBenchmark()
  console.table(terminalBenchmarkTable(result))
  console.info("idle", result.idle)
