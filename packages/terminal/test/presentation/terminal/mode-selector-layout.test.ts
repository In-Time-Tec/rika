...
} from "../../../src/presentation/terminal/mode-selector-layout"
...
test("lays out four non-overlapping mode labels across compact content widths 20 through 28", () => {
  for (let width = 20; width <= 28; width += 1) {
    const labels = modeSelectorLabels(width, modes)
...
test("abbreviates only the mathematically constrained 20-column medium label", () => {
  expect(modeSelectorLabels(20, modes)).toEqual([
  ...
  expect(modeSelectorLabels(21, modes).map((label) => label.text)).toEqual(["low", "medium", "high", "ultra"])
})
