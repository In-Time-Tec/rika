              ...
              Effect.runPromise(
                Effect.gen(function* () {
                  ...
                  const bounded = (
                    ...
                    expect(
                      renderable.x + renderable.width,
                      `${name} horizontal ${JSON.stringify(bounds)}`,
                    ).toBeLessThanOrEqual(width)
                    expect(
                      renderable.y + renderable.height,
                      `${name} vertical ${JSON.stringify(bounds)}`,
                      ...
                      if (surface.changedFilesBox.visible) {
                        ...
                        expect(
                          state.changedRows.every(
                            (row) =>
                              row.chunks.reduce((total, chunk) => total + stringWidth(chunk.text), 0) <= innerWidth,
                          ),
                        ).toBe(true)
                      }
            ...
            try {
              ...
              expect(
                setup
                  ...
                  .some(
                    (span) =>
                      span.text.includes("ERROR: Message failed") && span.fg.toInts().join(",") === "128,0,0,255",
                  ),
              ).toBe(true)
              yield* openTui(() => setup.mockInput.typeText("retry"))
              setup.mockInput.pressEnter()
