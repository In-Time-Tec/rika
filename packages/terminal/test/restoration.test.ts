...
it.effect("renders responsive context tracks and per-cell mode commit wipe colors", () =>
  Effect.gen(function* () {
    ...
    surface.update({
      ...
      contextUsage: { _tag: "Available", inputTokens: 50, inputCacheRead: 25, inputTotal: 50, contextWindow: 100, reserveTokens: 0 },
...
test("drains threshold flashes and compaction vacuum ticks only on animation ticks", () => {
  const before = {
    ...
    contextUsage: { _tag: "Available" as const, inputCacheRead: 0, inputTokens: 80, inputTotal: 80, contextWindow: 110, reserveTokens: 10 },
  }
  const compacted = { _tag: "Available" as const, inputTokens: 20, inputCacheRead: 0, inputTotal: 20, contextWindow: 110, reserveTokens: 10 }
  ...
  const threshold = { _tag: "Available" as const, inputTokens: 76, inputCacheRead: 0, inputTotal: 76, contextWindow: 110, reserveTokens: 10 }
  model = update(
...
it.effect("owns a continuous welcome cadence and stops it when transcript content mounts", () =>
  Effect.gen(function* () {
    ...
    clock.advance(3_200)
    expect(surface.animationDiagnostics().elapsedMillis).toBe(3_320)
    expect(globalTicks).toBe(0)
    expect(second).not.toBe(first)
    expect(renderedWelcome()).not.toBe(first)
    model = { ...model, entries: [{ role: "user", text: "hello" }] }
    surface.update(model)
    const stoppedAt = surface.animationDiagnostics().elapsedMillis
    expect(surface.animationDiagnostics().running).toBe(false)
    clock.advance(1_000)
    expect(surface.animationDiagnostics().elapsedMillis).toBe(stoppedAt)
    expect(globalTicks).toBe(0)
  }).pipe(Effect.scoped),
)
