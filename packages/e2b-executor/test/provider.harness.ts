...
const read = (path: string) =>
  Effect.flatMap(FileSystem.FileSystem, (fileSystem) =>
    fileSystem.readFileString(new URL(path, import.meta.url).pathname),
  )
  ...
  it.layer(BunServices.layer)((test) => {
    test("starts the exported remote executor host without controller credentials", () =>
      Effect.gen(function* () {
        ...
        const host = yield* read("../../remote-execution/src/host/service.ts")
        ...
        expect(host).toContain('const ptyReady = config.fence.target === "orb" && capabilities.pty')
        expect(host).toContain("pty: ptyReady")
        expect(host).toContain("browser: capabilities.browser")
        expect(host).toContain("services: capabilities.services")
        expect(host).toContain("Bun.env.RIKA_EXECUTOR_WORKSPACE_ROOT || RemoteRepositoryRoot")
