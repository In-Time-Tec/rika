...
describe("development E2B template", () => {
  it.effect("hashes the exact Docker build definition", () =>
    Effect.gen(function* () {
      ...
      const first = yield* Effect.promise(() => developmentTemplateSourceDigest(repositoryRoot))
      const second = yield* Effect.promise(() => developmentTemplateSourceDigest(repositoryRoot))
