...
This package is the E2B-only control-plane adapter. It owns explicit assignment, immutable template build selection, short-lived bootstrap and lease credentials, E2B lifecycle calls, verified filesystem checkpoint metadata, GitHub App installation-token brokering, and managed-orphan cleanup. `E2B_TEMPLATE_ID` selects the E2B template for `Sandbox.create`; `E2B_TEMPLATE_BUILD_ID` is the successful build receipt stored in assignment placement, executor identity, fencing, and inventory metadata. Before and after creation, the provider requires E2B's `default` template tag to resolve to that receipt and requires the receipt status to be `ready`; it also verifies the new sandbox's provider-reported template ID. Any mismatch is rejected before bootstrap, and a sandbox created during a retarget race is killed.
