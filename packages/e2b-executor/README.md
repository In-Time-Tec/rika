...
This package is the E2B-only API adapter. It owns explicit assignment, immutable template build selection, short-lived bootstrap and lease credentials, E2B lifecycle calls, verified filesystem checkpoint metadata, GitHub App installation-token brokering, and managed-orphan cleanup. `E2B_TEMPLATE_ID` identifies the E2B template; `E2B_TEMPLATE_BUILD_ID` is the successful build receipt. Sandbox creation uses E2B's exact-build selector `<template-id>:<build-id>`, and the receipt is also stored in assignment placement, executor identity, fencing, and inventory metadata. Before and after creation, the provider requires that exact receipt to have `ready` status and verifies the new sandbox's provider-reported template ID. It never trusts a mutable template tag. Any mismatch is rejected before bootstrap, and a sandbox created during a retarget race is killed.

It does not implement the product `ExecutionGateway`, Generalist RunStore, an agent loop, workspace tools, processes, or PTYs. Application composition must keep `ExecutionGateway` as the product boundary and Generalist as the sole durable Run authority. Executors receive neither Postgres credentials nor E2B/GitHub App controller credentials.
